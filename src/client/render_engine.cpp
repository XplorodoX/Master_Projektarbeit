...
SpriteId sandVariant(int wx, int wy) {
...
void drawStoneResource(int wx, int wy, int px, int py, int tileSize, Color biomeTint) {
    ...
    // Base stone color - adaptiert an Biom-Farbton
    Color baseColor = Color{
        static_cast<unsigned char>((140 * biomeTint.r) / 255),
        static_cast<unsigned char>((140 * biomeTint.g) / 255),
        static_cast<unsigned char>((145 * biomeTint.b) / 255),
        255
    };
    Color edgeColor = Color{
        static_cast<unsigned char>((100 * biomeTint.r) / 255),
        static_cast<unsigned char>((100 * biomeTint.g) / 255),
        static_cast<unsigned char>((105 * biomeTint.b) / 255),
        255
    };
    Color highlightColor = Color{
        static_cast<unsigned char>((180 * biomeTint.r) / 255),
        static_cast<unsigned char>((180 * biomeTint.g) / 255),
        static_cast<unsigned char>((190 * biomeTint.b) / 255),
        255
    };
    
    DrawRectangle(px, py, tileSize, tileSize, baseColor);
    ...
    if(stoneType == 0) {
        ...
        // Cracked stone with diagonal breaks
        const int crackCount = 3 + static_cast<int>(hash01(wx, wy, 4813) * 2.0F);
        for(int i = 0; i < crackCount; ++i) {
            const float angle = hash01(wx, wy, 4815 + i) * 6.2831853F;
            const float x1 = static_cast<float>(px + tileSize / 2) + std::cos(angle) * static_cast<float>(tileSize / 3);
            const float y1 = static_cast<float>(py + tileSize / 2) + std::sin(angle) * static_cast<float>(tileSize / 3);
            const float x2 = static_cast<float>(px + tileSize / 2) - std::cos(angle) * static_cast<float>(tileSize / 3);
            const float y2 = static_cast<float>(py + tileSize / 2) - std::sin(angle) * static_cast<float>(tileSize / 3);
            DrawLineEx(Vector2{x1, y1}, Vector2{x2, y2}, 1.5F, edgeColor);
        }
    } else if(stoneType == 2) {
        // Rough stone with spots
        for(int i = 0; i < 12; ++i) {
            const int sx = px + static_cast<int>(hash01(wx, wy, 4817 + i * 2) * static_cast<float>(tileSize));
            const int sy = py + static_cast<int>(hash01(wx, wy, 4819 + i * 2) * static_cast<float>(tileSize));
            const int size = 1 + static_cast<int>(hash01(wx, wy, 4821 + i) * 2.0F);
            DrawCircle(sx, sy, static_cast<float>(size), Fade(edgeColor, 0.4F));
        }
    } else {
        // Broken stone with jagged edges
        const int fragments = 6;
        for(int i = 0; i < fragments; ++i) {
            const float angle = static_cast<float>(i) * 6.2831853F / static_cast<float>(fragments);
            const int fx = px + tileSize / 2 + static_cast<int>(std::cos(angle) * tileSize / 3);
            const int fy = py + tileSize / 2 + static_cast<int>(std::sin(angle) * tileSize / 3);
            const int fsx = 2 + static_cast<int>(hash01(wx, wy, 4823 + i) * 3.0F);
            DrawCircle(fx, fy, static_cast<float>(fsx), Fade(highlightColor, 0.5F));
        }
        // Dark shadow in center
        DrawRectangle(px + tileSize / 4, py + tileSize / 4, tileSize / 2, tileSize / 2, Fade(edgeColor, 0.2F));
    }
...
void drawStyledTile(
    ...
    const bool nearWater = lakeOrAdjacentAt(sim, wx, wy);

    if(nearWater && !isSandBlockedTile(type)) {
...
void drawTileDetailPass(
    ...
    for(int vy = -viewRadiusY; vy <= viewRadiusY; ++vy) {
        for(int vx = -viewRadiusX; vx <= viewRadiusX; ++vx) {
            ...
            const bool northNeighborNearWater = lakeOrAdjacentAt(sim, wx, wy - 1);
            const bool southNeighborNearWater = lakeOrAdjacentAt(sim, wx, wy + 1);
            const bool westNeighborNearWater = lakeOrAdjacentAt(sim, wx - 1, wy);
            const bool eastNeighborNearWater = lakeOrAdjacentAt(sim, wx + 1, wy);
...
Color lakeTintColor() {
    return Color{68, 132, 201, 190};
}

void drawLakeTile(const stoneforge::Simulation& /*sim*/, int px, int py, int tileSize, float t) {
    // Base lake color
    ...
    for(int vy = -viewRadiusY; vy <= viewRadiusY; ++vy) {
        for(int vx = -viewRadiusX; vx <= viewRadiusX; ++vx) {
            ...
            if(sim.isLakeAt(wx, wy)) {
                drawLakeTile(sim, px, py, tileSize, t);
            }
        }
    }
...
int stoneforge::client::RenderEngine::run(bool aiMode, bool aiDualMode, std::uint64_t aiSeed,
    ...
    const char* winTitleC = winTitle.empty() ? "Stoneforge 2D - raylib biome client" : winTitle.c_str();
    InitWindow(initW, initH, winTitleC);
    if(winX >= 0 && winY >= 0) {
