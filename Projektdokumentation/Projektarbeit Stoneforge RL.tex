...
\bigskip
Da die Arbeit von zwei Personen gemeinsam erstellt wurde, wird zusätzlich
erklärt, dass die Abschnitte~3.1, 4.1 und~5.1 von Laurin~Rößler und die
Abschnitte~3.2, 4.2 und~5.2 von Florian~Merlau verfasst wurden; die
Kapitel~1 und~2 entstanden vollständig gemeinsam. Die
Zuordnung von Kapitel~6 und der Anhänge ist Anhang~B zu entnehmen.
...
% Zahlen abgeglichen mit Kapitel 5 / Anhang C (kanonischer Stand
% baselines.json, 5-Wiederholungs-Protokoll, siehe CHANGELOG
% v2026-08-14.2).
% ============================================================
\chapter*{Kurzfassung}
...
gemessen primär an der Erfolgsquote unter Berücksichtigung der resultierenden
Pfadeffizienz. Dazu wird eine C++-basierte Simulationsumgebung mit
...
statistisch signifikant. Die Pfadeffizienz bleibt bei beiden Verfahren
niedrig. Die $\varepsilon$-Kompass-Heuristik übertrifft bei
$\varepsilon = 0{,}8$ dennoch beide trainierten Modelle sowohl in der
Erfolgsquote als auch in der Pfadeffizienz. Dieser Befund wird methodisch
...
This work investigates, in two consecutive and mutually dependent parts, how a
procedurally generated, partially observable 2D world must be structured to
be fair and solvable for a reinforcement learning agent, and which training
procedure yields the best navigation results within it, measured primarily by
success rate while accounting for the resulting path efficiency. A custom
simulation environment implemented in C++ with a Python interface places the
exit of every world exclusively at positions confirmed reachable from the
spawn point by a breadth-first search, which structurally excludes unsolvable
world instances regardless of wall density. Recurrent and non-recurrent
Proximal Policy Optimization (PPO) agents are trained on this environment
through a performance-based curriculum and compared against an untrained
$\varepsilon$-compass heuristic baseline. Across seven independent runs, the
LSTM-PPO model reaches a mean success rate of 64.6\,\% ($\pm$ 10.1) on
Testset A and 68.8\,\% ($\pm$ 15.5) on Holdout B, statistically
significantly outperforming the MLP-PPO model (33.5\,\% $\pm$ 25.5 on
Testset A). Path efficiency remains low for both trained models. The
$\varepsilon$-compass heuristic at $\varepsilon = 0.8$ nonetheless
outperforms both trained models in success rate and path efficiency alike.
This finding is placed in a broader methodological context: the
reachability-optimized world generation itself may favor exactly the simple
path structure that the untrained heuristic exploits.
...
Dies macht die Kompass-Heuristik de facto \textbf{wandblind}. Sie nutzt keine Pfadfindungs-Logik (wie etwa den BFS-Gradienten) und kennt keine Hindernisse. Sie rennt stumpf gegen Wände, bis der Zufallsanteil $\varepsilon$ sie wieder löst. Dass ein komplett kartenblindes Verfahren in der Evaluation derart stark abschneidet, ist eine zentrale Erkenntnis für die spätere Auswertung und erklärt zugleich, warum bei dieser Baseline im getesteten Bereich $\varepsilon \in [0{,}3; 0{,}9]$ mit steigendem Zufallswert $\varepsilon$ die Erfolgsquote steigt und die Pfadeffizienz zugleich sinkt. Dieser Zusammenhang gilt jedoch nicht uneingeschränkt: Bei $\varepsilon=1$ entspricht die Heuristik exakt dem Random-Agent, dessen Erfolgsquote von 5,2\,\% (siehe Abbildung~\ref{fig:eval_sr}) weit unter dem Wert bei $\varepsilon=0{,}9$ liegt, sodass die Kurve zwischen beiden Randwerten einbrechen muss.
...
\label{sec:eval_eff}
Nur am Ausgang anzukommen, reicht als Bewertungskriterium nicht aus, wichtig ist auch der Weg dorthin. Abbildung~\ref{fig:eval_eff} zeigt daher die Pfadeffizienz. Sie wird pro erfolgreicher Episode als Verhältnis zwischen dem per Breitensuche (siehe Abschnitt~\ref{sec:grundlagen_bfs}) ermittelten kürzesten Weg dieser Episode und den vom Agenten tatsächlich gelaufenen Schritten berechnet und anschließend über alle erfolgreichen Episoden gemittelt. Da dieser Mittelwert der Verhältnisse nicht dem Verhältnis der Mittelwerte entspricht, lässt sich die berichtete Pfadeffizienz nicht durch Division der weiter unten genannten Durchschnittswerte für BFS-Optimum und Schrittzahl nachrechnen. Ein Wert von 1,0 markiert das Optimum.
...
Das BFS-Optimum liegt auf dem verwendeten Testset im Schnitt bei 40,1 Schritten. Das trainierte LSTM-Modell benötigt für eine erfolgreiche Episode dagegen durchschnittlich 1425 Schritte, bei einer gemittelten Pfadeffizienz von lediglich 0,053. Der Agent läuft damit im Schnitt einen Weg, der mehr als 35-mal so lang ist wie nötig. Abbildung~\ref{fig:pfad_vs_bfs} macht diesen Unterschied an einer einzelnen Episode aus Testset A sichtbar.
...
Der abgebildete Lauf erreicht mit 456 gegenüber 43 BFS-optimalen Schritten eine Pfadeffizienz von 0,094 und liegt damit über dem berichteten Mittel von 0,053; die wiederholten Schleifen im eingezeichneten Pfad zeigen aber bereits an dieser einzelnen, vergleichsweise günstigen Episode das charakteristische Herumirren, das die niedrige mittlere Pfadeffizienz erklärt. Für das MLP-Modell liegt ein vergleichbarer Wert vor: Bei durchschnittlich 1084 Schritten je erfolgreicher Episode liegt die gemittelte Pfadeffizienz bei 0,067 und damit höher als beim LSTM. Dieser Befund ist mit Vorsicht zu lesen, da die Kennzahl nur über die erfolgreichen Episoden gemittelt wird, wie der folgende Absatz zum Selektionseffekt zeigt: Bei einer Erfolgsquote von nur 33,5\,\% löst das MLP-Modell bevorzugt die leichten Seeds, was die Effizienz künstlich anhebt.

Aufschlussreich wird der Blick auf die Referenzverfahren aus Abschnitt~\ref{sec:methodik_baselines}, deren vollständige Ergebnistabelle in Anhang~C aufgeführt ist. Der vollständig zufällig agierende Random-Agent benötigt in seinen seltenen erfolgreichen Episoden im Schnitt 2251,9 Schritte, bei einer gemittelten Pfadeffizienz von lediglich 0,021. Der ungelernte Kompass-Agent mit einem Zufallsanteil von $\varepsilon=0{,}3$ erreicht eine Effizienz von 0,177 und ist damit rund 3,3-mal so wegeffizient wie das trainierte LSTM-Netz, auch wenn er dabei eine rund 13 Prozentpunkte niedrigere Erfolgsquote in Kauf nimmt. Das überraschendere Ergebnis liefert der $\varepsilon=0{,}8$-Kompass: Er übertrifft das trainierte Modell nicht nur bei der Erfolgsquote, sondern mit einer Pfadeffizienz von 0,094 gleichzeitig auch bei der Weglänge.
...
\begin{table}[H]
    \centering
    \small
    \caption{Ungelernte Referenzverfahren: Erfolgsquote, mittlere Schrittzahl und Pfadeffizienz, Testset A und Holdout B}
    \label{tab:rohdaten_referenz}
    \begin{tabular}{lrrrrrr}
        \toprule
        & \multicolumn{3}{c}{\textbf{Testset A}} & \multicolumn{3}{c}{\textbf{Holdout B}} \\
        \textbf{Verfahren} & \textbf{SR (\%)} & \textbf{Schritte} & \textbf{Effizienz} & \textbf{SR (\%)} & \textbf{Schritte} & \textbf{Effizienz} \\
        \midrule
