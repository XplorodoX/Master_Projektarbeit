...
% ============================================================
\chapter*{Hinweis zur Verwendung von KI-Werkzeugen}
\addcontentsline{toc}{chapter}{Hinweis zur Verwendung von KI-Werkzeugen}
\thispagestyle{empty}

Bei der Erstellung dieser Arbeit wurden KI-gestützte Werkzeuge (GitHub Copilot
sowie Claude von Anthropic) als Hilfsmittel eingesetzt. Sie dienten der
sprachlichen Ausformulierung und Überarbeitung einzelner Textpassagen sowie
als Unterstützung bei der Programmierung. Die inhaltlichen Aussagen, die
Konzeption, die Durchführung der Experimente und sämtliche berichteten
Ergebnisse wurden eigenständig erarbeitet; alle KI-gestützt formulierten
Passagen wurden geprüft und werden inhaltlich verantwortet.
...
\begin{center}
\itshape »Stoneforge RL — Navigation in einer eigenentwickelten prozedural\\
generierten 2D Welt mittels Reinforcement Learning«
\end{center}
...
Die vorliegende Arbeit untersucht in zwei aufeinander aufbauenden Teilen, wie
eine prozedural generierte, partiell beobachtbare 2D-Welt aufgebaut sein muss,
damit sie für einen Reinforcement-Learning-Agenten fair und lösbar ist, und
welches Trainingsverfahren darin die besten Navigationsergebnisse liefert,
gemessen primär an der Erfolgsquote unter Berücksichtigung des resultierenden
Wegverhältnisses. Dazu wird eine C++-basierte Simulationsumgebung mit
angebundenem Python-Interface entwickelt, in der der Ausgang jeder Welt
ausschließlich über eine Breitensuche vom Spawnpunkt aus platziert wird;
unlösbare Weltinstanzen werden dadurch strukturell ausgeschlossen, unabhängig
von der Wanddichte. Auf dieser Umgebung werden rekurrente und
nicht-rekurrente Proximal-Policy-Optimization-Modelle (PPO) über ein
leistungsbasiertes Curriculum trainiert und gegen eine ungelernte
$\varepsilon$-Kompass-Heuristik eingeordnet. Über sieben unabhängige Läufe
erreicht das LSTM-PPO-Modell eine mittlere Erfolgsquote von 64,6\,\%
($\pm$ 10,1) auf Testset A und 68,8\,\% ($\pm$ 15,5) auf Holdout B und
übertrifft damit das MLP-PPO-Modell (33,5\,\% $\pm$ 25,5 auf Testset A)
statistisch signifikant. Das Wegverhältnis bleibt bei beiden Verfahren
niedrig. Die $\varepsilon$-Kompass-Heuristik übertrifft bei
$\varepsilon = 0{,}8$ dennoch beide trainierten Modelle sowohl in der
Erfolgsquote als auch im Wegverhältnis. Dieser Befund wird methodisch
eingeordnet; als mögliche Teilerklärung wird herausgearbeitet, dass die auf
Erreichbarkeit optimierte Weltgenerierung selbst die einfache Wegstruktur
begünstigen könnte, die das ungelernte Verfahren ausnutzt.
...
Die Arbeit verbindet zwei Themenfelder, die sich in der Praxis selten in einem gemeinsamen Projekt begegnen: prozedurale Spielweltgenerierung und Reinforcement Learning. Beide Bereiche greifen ineinander, sobald eine Spielwelt nicht mehr von Hand entworfen, sondern algorithmisch erzeugt wird und ein lernender Agent auf dieser Grundlage navigieren soll.

Als Vorbild dienten Spiele wie \textit{Minecraft}, deren Reiz zu einem großen Teil darin liegt, dass Welten bei jedem Start prozedural neu erzeugt werden. Ziel dieser Arbeit ist es, ein ähnliches Prinzip im Kleinen umzusetzen: Eine einfache, prozedural generierte Spielwelt zu bauen und darauf einen RL-Agenten zu trainieren, der lernen muss, sich in dieser unbekannten Umgebung zurechtzufinden und den Ausgang zu erreichen.
...
Diese Einschränkung wurde bewusst eingebaut: Wenn der Agent immer die komplette Karte auf einmal sehen könnte, wäre die Aufgabe deutlich zu einfach. Das ließe sich dann bereits mit wenigen festen Regeln oder einfachen Algorithmen lösen, ohne dass ein RL-Agent tatsächlich etwas lernen müsste.

Durch die eingeschränkte Sicht muss der Agent jedoch mit unvollständigem Wissen arbeiten, etwa so wie ein Mensch, der sich ohne Karte durch ein völlig unbekanntes Gebäude tastet. Der Agent muss also durch eine ihm unbekannte Welt navigieren, die er erst beim Durchlaufen Schritt für Schritt entdeckt.
...
Wie muss eine prozedural generierte 2D-Spielwelt aufgebaut sein und welche Kombination aus Policy-Architektur und Trainingsverfahren erzielt darin die besten Ergebnisse bei der Navigation?

\paragraph{Konkret gliedert sich dies in zwei Teilfragen:}

\begin{itemize}
    \item \textbf{Weltdesign:} Wie lässt sich eine performante, seedbasierte, abwechslungsreiche 2D-Gridwelt erstellen, die durch ein Ziel lösbar ist und deren Grundmechaniken gut für eine KI lernbar sind?
    \item \textbf{Algorithmen \& Parameter:} Welche Kombination aus Policy-Architektur und Trainingsverfahren erreicht die höchste Erfolgsquote und die besten Wege zum Ausgang?
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Prototyping der Spielumgebung:} Auf dieser Basis entsteht ein erster Prototyp der Spielwelt. Dieser wird iterativ verbessert, bis Parameter wie Exit-Distanz, Wanddichte und die Sichtweite des Agenten gut zusammenpassen und die Level fair bleiben.
    ...
    \item \textbf{Evaluation und Auswertung:} Zum Schluss werden die fertig trainierten Agenten auf vielen neu generierten Leveln getestet. Die Ergebnisse, etwa Erfolgsquote und Wegverhältnis, werden systematisch erfasst und die Verfahren miteinander verglichen.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Begrenzte Parameterstudie:} Auf eine erschöpfende Testreihe über alle denkbaren Kombinationen aus Wanddichten oder Exit-Distanzen wird verzichtet; der Fokus liegt auf den ausgewählten RL-Verfahren und praxisnahen Welteinstellungen.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Partielle Beobachtbarkeit und Gedächtnis (POPGym):} Besonders nah am RL-Teil dieser Arbeit ist \textit{POPGym} \cite{morad2023popgym}. Dort werden ganz gezielt Umgebungen mit eingeschränkter Sicht (partielle Beobachtbarkeit) untersucht und es wird verglichen, wie sich Modelle mit Gedächtnis (rekurrente Policies) gegen Modelle ohne Gedächtnis schlagen. Das ist exakt der Ansatz, der auch für den Vergleich der Trainingsverfahren in dieser Arbeit übernommen wird.
...
\begin{itemize}
    \item \textbf{Multi-Frequency / Fraktaler Noise:} Durch die Addition mehrerer Rauschfunktionen unterschiedlicher Frequenzen (Oktaven) und Amplituden lassen sich fraktalähnliche, hochdetaillierte Strukturen erzeugen. Niedrige Frequenzen steuern großflächige Formen (zum Beispiel Kontinente), während hohe Frequenzen feine Details (zum Beispiel Felsstrukturen) ergänzen.
...
\begin{figure}[htbp]
    ...
    \caption{Zellulärer Automat -- Rauschgitter vor und nach vier statt der konfigurierten zwei Glättungsiterationen, um den Effekt deutlicher zu zeigen, eigene Darstellung.}
...
Zur Analyse der Konnektivität wird das Zellgitter als ungerichteter Graph interpretiert, in dem begehbare Zellen als Knoten und Nachbarschaftsbeziehungen als Kanten dargestellt werden. Die Breitensuche (BFS) traversiert den Graphen ebenenweise vom Startknoten aus \citep{clrs2009algorithms}. Sie eignet sich zur Prüfung der Erreichbarkeit eines Zielknotens sowie zur Bestimmung des kürzesten Pfades auf ungewichteten Gittern. Abbildung~\ref{fig:grundlagen_bfs} veranschaulicht das resultierende Distanzfeld an einem Beispielgitter: Jede Ebene entspricht einem BFS-Schritt vom Start aus, der kürzeste Pfad zum Ziel ergibt sich durch Rückverfolgung der jeweils näher liegenden Nachbarzelle.

Im ausgelieferten Konfigurationsstand dient die Breitensuche nicht mehr der nachträglichen Erreichbarkeitsprüfung bei der Weltgenerierung (siehe Tabelle~\ref{tab:config_status}); diese Validierung ist deaktiviert. Lösbarkeit wird stattdessen vorgelagert erzwungen, indem der Ausgang selbst nur unter Positionen gewählt wird, die eine Breitensuche vom Spawnpunkt aus als erreichbar bestätigt (siehe Abschnitt~\ref{sec:umsetzung_pathing}). Die Breitensuche bleibt damit an mehreren Stellen aktiv: bei der Ausgangsplatzierung, als Distanzfeld für das Reward Shaping im RL-Training (Abschnitt~\ref{sec:impl_rl_reward}) und als Bezugsgröße der Wegverhältnis-Metrik in der Evaluation (Abschnitt~\ref{sec:eval_eff}).
...
\begin{figure}[htbp]
    ...
    \caption{Veranschaulichung des Potentialfelds $\Phi(s)$ und der daraus resultierenden formenden Belohnung entlang eines Beispielpfads, eigene Darstellung.}
...
Bevor das Training beginnt, muss der Agent wissen, welche Sensordaten er empfängt und welche Aktionen ihm zur Verfügung stehen. Gymnasium definiert dafür sogenannte \texttt{spaces}:
\begin{itemize}
    \item \texttt{observation\_space}: Definiert das Format und die Wertebereiche der Beobachtungen, die die Umgebung zurückgibt (zum Beispiel ein mehrdimensionaler Vektor für Kamerabilder oder Sensordaten, repräsentiert als \texttt{Box}-Raum).
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{terminated}: Ein boolescher Wahrheitswert, der angibt, ob die Episode durch einen regulären Endzustand (zum Beispiel Ziel erreicht oder \glqq Game Over\grqq) beendet wurde.
        \item \texttt{truncated}: Ein boolescher Wahrheitswert, der angibt, ob die Episode künstlich von außen abgebrochen wurde (zum Beispiel weil ein Zeitlimit beziehungsweise eine maximale Schrittzahl erreicht wurde).
...
Zur zustandslosen Berechnung von Raumkoordinaten $(x, y)$ transformiert eine Hash-Funktion die Koordinaten, den Welt-Seed und ein Salt in einen normalisierten Zufallswert im Intervall $[0, 1]$.

Für zusammenhängende Strukturen wird ein \textit{Value-Noise}-Verfahren verwendet, dessen theoretische Grundlage bereits vorgestellt wurde (siehe Kapitel~2.1.2). Die Werte der vier Eckpunkte einer Gitterzelle werden ermittelt und bilinear interpoliert. Zur Kantenenglättung wird der Interpolationsparameter $t \in [0, 1]$ zuvor über die bereits eingeführte Smoothstep-Funktion (Gleichung~\ref{eq:smoothstep}) transformiert.

\subsubsection{Domain Warping und fraktaler Noise}
Um sichtbare Rasterstrukturen des quadratischen Gitters zu brechen, verzerrt ein vorgeschaltetes Rauschen die Eingangskoordinaten (\textit{Domain Warping}). Zusätzlich werden mehrere Rauschfunktionen unterschiedlicher Frequenzen und Amplituden aufaddiert (fraktaler Noise), um feiner strukturierte Umgebungen zu erzeugen.

\subsubsection{Nachbearbeitung und Validierung}
Die generierten Chunks können optional mittels zellulärer Automaten nachbearbeitet werden. Die Geburts- und Überlebensregeln der Moore-Nachbarschaft werden dabei über die Konfiguration gesteuert.

Zur topologischen Prüfung kann optional eine BFS eingesetzt werden (siehe Kapitel~2.1.4), die die Erreichbarkeit innerhalb des Chunks sowie valide Ausgänge in einem definierten Distanzring verifiziert. Im ausgelieferten Konfigurationsstand ist diese Prüfung deaktiviert; warum die Deaktivierung die Lösbarkeit unberührt lässt, zeigt Abschnitt~\ref{sec:konfiguration}.

\subsection{Heuristiken und Regeln}
\label{sec:heuristiken}

\subsubsection{Biome und Schwellenwerte}
Die Welt wird über ein Binning des Rauschergebnisses in festen Intervallen von $\frac{1}{7}$ in sieben Biome unterteilt. Biomspezifische Schwellenwerte steuern das Platzieren von Hindernissen, Erzen und Vegetation.

Gewässer entstehen durch eine gewichtete Linearkombination zweier niederfrequenter Rauschfunktionen:
\begin{equation}
\text{Score} = 0,6 \cdot a + 0,4 \cdot b
\end{equation}
Zellen mit einem Wert über $0,86$ werden als Wasser markiert.

\subsubsection{Freiräume und Fallback-Pfade}
Die eigentliche Lösbarkeitsgarantie entsteht bereits bei der Wahl des Ausgangs: Kandidaten werden per Breitensuche vom Spawnpunkt aus ermittelt, sodass ausschließlich nachweislich erreichbare Positionen als Ausgang infrage kommen. Zusätzlich steht optional ein Fallback-Algorithmus bereit, der bei Bedarf einen direkten Manhattan-Pfad (zuerst X-, dann Y-Achse) in die Karte gräbt; im ausgelieferten Konfigurationsstand ist dieser Fallback deaktiviert (siehe Abschnitt~\ref{sec:konfiguration}). Feste Radien um Spawn- und Zielpunkte verhindern zusätzlich das Blockieren der Start- und Endpositionen.

Point-of-Interest-Strukturen werden mit einer Wahrscheinlichkeit von $P = 0,10$ pro Chunk platziert. Ihre Form basiert auf biomspezifischen $5 \times 5$ ASCII-Matrizen.

\subsection{Systemkonfiguration}
\label{sec:konfiguration}

Die früher wegen des festen $\frac{1}{7}$-Binnings funktional obsoleten Parameter \texttt{coldBiomeMax} und \texttt{warmBiomeMax} wurden inzwischen vollständig entfernt (Parser, Struct-Felder und JSON-Schlüssel); sie tauchen deshalb auch nicht mehr im Konfigurationsauszug in Anhang~A auf. \texttt{enableMacroGraphPrecheck} und \texttt{macroGraphRadiusChunks} werden dagegen weiterhin eingelesen, aber im Quellcode an keiner Stelle ausgewertet; sie sind damit tote Konfigurationsschlüssel ohne Wirkung.

Der Manhattan-Carver aus Abschnitt~\ref{sec:umsetzung_pathing} lässt sich über zwei unabhängige Schalter auslösen: \texttt{forceGuaranteedPath} ruft ihn unbedingt bei jedem \texttt{reset()} auf, während \texttt{guaranteedPathFallback} ihn nur als nachgelagerte Korrektur einsetzt, falls der optionale Flood-Fill-Erreichbarkeitstest (\texttt{enableFloodFillValidation}) eine unerreichbare Welt meldet. Da Letzterer im ausgelieferten Konfigurationsstand ebenfalls deaktiviert ist, bleibt \texttt{guaranteedPathFallback} praktisch wirkungslos. Beide Schalter sind im ausgelieferten Konfigurationsstand deaktiviert.

Der aktuelle Standard-Betriebsmodus laut Konfigurationsdatei:
...
\small
\begin{tabular}{|p{7.6cm}|p{2.4cm}|p{2.6cm}|}
\hline
\textbf{Feature / Algorithmus} & \textbf{Status} & \textbf{Referenz} \\ \hline
Zelluläre Automaten (CA) & DEAKTIVIERT & siehe Anhang~A \\ \hline
Erreichbarkeitstest (Flood-Fill) & DEAKTIVIERT & siehe Anhang~A \\ \hline
Manhattan-Carving-Fallback (\texttt{forceGuaranteedPath}) & DEAKTIVIERT & siehe Anhang~A \\ \hline
Manhattan-Carving-Fallback, Flood-Fill-Korrektur (\texttt{guaranteedPathFallback}) & DEAKTIVIERT & siehe Anhang~A \\ \hline
\end{tabular}
\end{table}

Die in Tabelle~\ref{tab:config_status} aufgeführten Mechanismen sind im ausgelieferten Konfigurationsstand sämtlich deaktiviert. Die Lösbarkeit wird dennoch unbedingt sichergestellt, jedoch nicht durch einen dieser Schalter, sondern durch die Konstruktion der Ausgangsplatzierung selbst: Der Ausgang wird ausschließlich unter Positionen gewählt, die eine Breitensuche vom Spawnpunkt aus als erreichbar bestätigt (siehe Abschnitt~\ref{sec:umsetzung_pathing}). Diese BFS-Exit-Platzierung ist fest im Code verankert und nicht über die Konfiguration abschaltbar.
...
Im Mittelpunkt des Vergleichs stehen zwei Varianten von PPO: eine mit rekurrenter LSTM-Policy (im Folgenden LSTM-PPO) und eine mit zustandsloser Multilayer-Perceptron-Policy (MLP-PPO). Da beide Modelle denselben Optimierungsmechanismus nutzen, lässt sich durch diesen Aufbau gut untersuchen, welchen Einfluss das Gedächtnis (Rekurrenz) auf die Navigation in solchen Umgebungen hat. 

Frühere Tests mit Deep Q-Networks (DQN) wurden für diese Untersuchung verworfen. Erste Analysen zeigten, dass die gelernten Q-Werte für alle vier Aktionen extrem nah beieinander lagen und sich damit kaum unterschieden. Dadurch konnte der Agent keine klare Richtung lernen und hing oft in endlosen Pendelschleifen zwischen zwei Zellen fest. Ein weiteres Problem war der Replay-Puffer von DQN: Falsch berechnete Rewards aus den ersten Durchläufen blieben darin gespeichert und störten das spätere Training immer wieder. Da PPO als On-Policy-Verfahren solche alten Daten direkt verwirft, treten diese historischen Verzerrungen gar nicht erst auf. Der A2C-Algorithmus wurde theoretisch ebenfalls in Betracht gezogen, wird hier aber nicht praktisch getestet. Um die Leistung der PPO-Agenten stattdessen besser einschätzen zu können, dienen drei ungelernte Baselines (zwei Heuristiken und ein Orakel) als direkter Vergleich.

\subsection{Problemformulierung: Zustand, Aktionsraum und Reward}
\label{sec:methodik_problemformulierung}

Passend zu den theoretischen Grundlagen wird die Navigationsaufgabe als POMDP modelliert (siehe Kapitel~2.2). Die wichtigsten Bausteine dazu sind in Tabelle~\ref{tab:pomdp_komponenten} zusammengefasst. Der Agent sieht also nicht die komplette Karte ($s \in S$), sondern bekommt in jedem Schritt nur einen lokalen Ausschnitt $o \in \Omega$. Das ist konkret ein 15$\times$15 Zellen großes Sichtfeld plus einige Zusatzinformationen, wie die relative Richtung zum Ziel.

Die Größe des Sichtfelds ist dabei ein Zielkonflikt: Ein zu kleiner Ausschnitt lässt den Agenten Hindernisse zu spät erkennen, ein zu großer lässt den Beobachtungsraum unnötig anwachsen und untergräbt zugleich den Charakter der partiellen Beobachtbarkeit, auf den es in dieser Arbeit gerade ankommt. Der konkrete Wert von 15 Feldern liegt bewusst im mittleren Bereich dieses Kompromisses; eine systematische Rastersuche über alternative Radien war im Rahmen dieser Arbeit nicht vorgesehen. Daraus entsteht ein Beobachtungsvektor mit 229 Dimensionen. Als Aktionen gibt es vier diskrete Bewegungsrichtungen. Auf diagonale Schritte wurde verzichtet, da dies im rasterbasierten Labyrinth der Spielumgebung zu Problemen mit den Wänden führt.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{4cm}p{9.8cm}}
        ...
        \midrule
        Beobachtungsraum ($\Omega$) & Lokaler 15$\times$15 Grid-Ausschnitt, Trefferpunkte, Zielrichtung, Schrittbudget-Anteil (229 Dimensionen) \\
...
Die Reward-Funktion wurde bewusst einfach gehalten. Das primäre Signal bleibt „sparse“: Der weitaus größte Ausschlag ist die einzelne, große Belohnung, wenn der Agent den Ausgang erreicht. Auf hohe Schrittstrafen wird verzichtet, damit der Agent nicht durch ständige große Abzüge vom eigentlichen Ziel abgelenkt wird; ein minimaler Zeitabzug von $-0{,}01$ pro Schritt bleibt dabei bestehen (siehe Abschnitt~\ref{sec:impl_rl_reward}). Um das Training dennoch etwas zu beschleunigen, kommt das Potential-Based Reward Shaping (PBRS) nach Ng, Harada und Russell zum Einsatz (siehe Kapitel~2.2). Mit der formenden Belohnung $F(s,a,s') = \gamma\Phi(s') - \Phi(s)$ und einem BFS-Distanzfeld als Potenzialfunktion $\Phi$ ist die Optimalpolitik-Invarianz nach \citet{ng1999policy} mathematisch sichergestellt. Diese Garantie gilt jedoch nur für den PBRS-Anteil selbst, nicht für die Gesamtbelohnung: Der tatsächliche Reward enthält mit dem Neuland-Bonus, der Loop-Strafe und der Besuchsstrafe des Python-Wrappers weitere, nicht-potenzialbasierte Terme (siehe Abschnitt~\ref{sec:impl_rl_reward}), die die optimale Politik durchaus verändern können.

\subsection{Curriculum Konzept und Schnittstelle zur Weltgenerierung}
\label{sec:methodik_curriculum}

Damit das Training nicht sofort an der vollen Zieldistanz scheitert, ist der Lernprozess in ein vierphasiges Curriculum unterteilt (siehe Abbildung~\ref{fig:curriculum_phasen}). In den Phasen wird die sogenannte Exit-Distanz, also der minimale Laufweg vom Start zum Ausgang, schrittweise erhöht. Das setzt technisch zwingend voraus, dass der Weltgenerator einen Regler bietet, mit dem sich der Ausgang gezielt in einem bestimmten Radius platzieren lässt (siehe Kapitel~3.1). Die Trainingspipeline überschreibt dazu bei jeder Phase die statischen Werte \texttt{exitMinDistance} und \texttt{exitMaxDistance} der Konfigurationsdatei (siehe Anhang~A) programmatisch mit den jeweiligen Phasengrenzen; der dort hinterlegte Bereich von 35 bis 45 Schritten gilt ausschließlich für den in Abschnitt~\ref{sec:methodik_protokoll} beschriebenen Eval-Prozess.

Phase 1 startet mit kurzen Wegen von 5 bis 12 Schritten, die in Phase 2 auf 12 bis 25 und in Phase 3 auf 25 bis 45 Schritte ansteigen. In Phase 4 bleibt die maximale Distanz bei 25 bis 45 Schritten gleich, aber die Explorationsrate des Agenten wird gezielt heruntergefahren. Den vier Phasen kommt dabei jeweils eine eigene Rolle zu: Phase 1 vermittelt das elementare Navigationsprinzip auf kurzen, leicht lösbaren Strecken, Phase 2 überträgt dieses Prinzip auf mittlere Distanzen und prüft damit erstmals die Generalisierungsfähigkeit, Phase 3 stellt die volle Zielverteilung her und beginnt zugleich die Determinisierung der Politik, und Phase 4 dient als reines Greedy Fine-Tuning, das gezielt die Lücke zwischen deterministischem und stochastischem Modus schließen soll.

Sowohl diese Distanzbereiche als auch die phasenspezifischen Ziel-Erfolgsquoten von 85/70/70/60\,\% (siehe Abschnitt~\ref{sec:impl_rl_pipeline}) wurden empirisch so gewählt, dass ein zuverlässiges Meistern von Phase 1 die Voraussetzung für einen vorzeitigen Phasenwechsel bilden sollte; eine systematische Optimierung der exakten Zahlenwerte war im Rahmen dieser Arbeit nicht vorgesehen, wohl aber die hier beschriebene funktionale Aufteilung der Phasen. In der Praxis erreichte dieses Gate jedoch nur einer der sieben LSTM-PPO-Läufe, die übrigen sechs wechselten nach Ausschöpfen des Schrittbudgets in Phase 2, ohne die Zielquote zweimal in Folge erreicht zu haben (siehe Abschnitt~\ref{sec:impl_rl_pipeline}). Dadurch soll der Agent aus der anfänglichen Probierphase eine feste, deterministische Strategie entwickeln.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{Bilder/curriculum_phasen.png}
\caption{Struktur des vierphasigen Curriculum-Trainings mit Steigerung der Exit-Distanz, eigene Darstellung.}
\label{fig:curriculum_phasen}
\end{figure}

Wichtig für den Trainingserfolg ist, dass der Wechsel zwischen den Phasen über ein duales Gate abläuft: Die nächste Phase startet entweder beim Erreichen einer bestimmten Erfolgsquote in zwei aufeinanderfolgenden Evaluationen, oder sobald das für die Phase vorgesehene Schrittbudget aufgebraucht ist. Ein rein zeitbasierter Wechsel wird dadurch vermieden, ohne das Training bei ausbleibendem Erfolg unbegrenzt in einer Phase zu belassen.
...
\begin{figure}[htbp]
    ...
    \caption{Entscheidungsregel und ausgewertete Parameter der $\varepsilon$-Kompass-Heuristik, eigene Darstellung.}
    \label{fig:epsilon_kompass}
\end{figure}

Das obere Limit bildet das BFS-Optimum. Dieses Orakel nutzt eine Breitensuche, um immer den mathematisch kürzesten Weg zu finden, und zeigt damit das absolute Maximum an Pfadqualität auf. Das untere Limit ist der Random-Agent, der völlig zufällige Aktionen wählt. Dazwischen liegt die $\varepsilon$-Kompass-Heuristik: Sie wählt mit einer Wahrscheinlichkeit von $\varepsilon$ einen zufälligen Schritt und bewegt sich ansonsten geradlinig in Richtung Ziel. Diese Heuristik wurde mit verschiedenen Werten getestet ($\varepsilon \in \{0{,}3; 0{,}5; 0{,}6; 0{,}8; 0{,}9\}$), um aufzuzeigen, wie Erfolgsquote und Streckenlänge in dieser Umgebung typischerweise zusammenhängen. Die Stützstellen decken dabei einen niedrigen, mittleren und hohen Zufallsanteil ab; sie entstammen keinem systematisch gleichmäßigen Raster, sondern wurden im Projektverlauf schrittweise um zusätzliche Werte ergänzt, ohne dass eine abschließende Rastersuche über das gesamte Intervall durchgeführt wurde.

\subsection{Trainings- und Evaluationsprotokoll}
\label{sec:methodik_protokoll}

Um verlässliche und reproduzierbare Ergebnisse zu erhalten, wurde das Vorgehen für das Training und die Evaluation strikt festgelegt. Das Training lief über sieben unabhängige Zufalls-Seeds; ursprünglich war aus Zeitgründen eine engere Grenze von drei Läufen geplant, die sich angesichts des gemessenen Durchsatzes (siehe Abschnitt~\ref{sec:impl_rl_netze}) als überflüssig erwies, da das LSTM-Training als langsamere der beiden Architekturen sieben Wiederholungen im vorgesehenen Zeitrahmen zuließ. Danach wurden die Modelle auf zwei komplett neuen Umgebungssets getestet, die während des Trainings nicht vorkamen: dem regulären Testset A (Seeds 7000–7049) und einem zusätzlichen Holdout B (Seeds 8000–8049), um die Generalisierungsfähigkeit zu prüfen. Die trainingsbegleitenden Evaluationen, die während des Trainings selbst über das in Abschnitt~\ref{sec:impl_rl_pipeline} beschriebene Gating entscheiden, laufen dagegen auf einem dritten, von beiden Testsets unabhängigen Seed-Bereich, den sogenannten Validierungs-Seeds (6000–6049). Die sieben resultierenden LSTM-PPO-Modelle werden im weiteren Verlauf unter der Kurzbezeichnung v12 (v12\_s1 bis v12\_s7) geführt, dem finalen, für diese Arbeit berichtsfähigen Konfigurations- und Trainingsstand.

Jedes der beiden je 50 Seeds umfassenden Testsets wird dabei nicht nur einmal durchlaufen, sondern über fünf unabhängige Wiederholungen mit je eigenem Politik-RNG-Seed ausgewertet, während die Welt-Seeds selbst fix bleiben; die berichteten Prozentwerte sind Mittelwerte über diese fünf Wiederholungen. Die feinere Auflösung mancher Prozentangaben (etwa 65,2\,\%) erklärt sich dadurch: Jede der fünf Wiederholungen liefert für sich einen Wert im 2-Prozent-Raster von 50 Seeds, der Mittelwert über fünf solcher Werte kann jedoch Vielfache von 0,4 Prozentpunkten annehmen.

Da ein rein zufällig agierender Agent in den seltenen erfolgreichen Fällen im Schnitt 2251,9 Schritte bis zum Ziel benötigt (siehe Abschnitt~\ref{sec:eval_eff}), war für die Tests ein festes Episodenlimit zwingend erforderlich. Ohne ein solches Limit ließe sich die Erfolgsquote nicht sinnvoll berechnen, da ein schlechter Agent sonst unendlich lange im Labyrinth verweilen würde. Überschreitet ein Agent das Limit, gilt der Versuch strikt als gescheitert. Als zweiter, seltenerer Scheiter-Modus gilt der in Abschnitt~\ref{sec:impl_rl_parameter} beschriebene Early-Stop nach 256 Schritten ohne positiven Reward: Er ist fest im Umgebungs-Wrapper verankert und greift damit auch in den Testset-A/B-Messungen, nicht nur während des Trainings. 

Zusätzlich werden die Modelle zweigleisig betrachtet: einmal deterministisch (der Agent wählt immer die Aktion mit der höchsten Wahrscheinlichkeit) und einmal stochastisch (die Aktion wird anhand der gelernten Verteilung gesampelt). Erst dieser Vergleich zeigt, ob das Netzwerk eine belastbare Richtungsstrategie verstanden hat oder lediglich durch stochastisches Raten ans Ziel gelangt. Die für Testset A und B berichtete Haupterfolgsquote (siehe Abschnitt~\ref{sec:eval_sr}) ist dabei durchgehend die stochastische, da sie dem Verhalten der Policy während des eigentlichen Trainings entspricht; das kanonische Eval-Skript deckt ausschließlich diesen Modus ab. Eine ergänzende deterministische Auswertung liegt als separate Zusatzmessung auf denselben Testset-Seeds nur für das LSTM-PPO-Modell vor; für das MLP-PPO-Modell wird ausschließlich die stochastische Erfolgsquote berichtet (siehe Abschnitt~\ref{sec:eval_sr}).

Bei den Metriken ist die Erfolgsquote (Success Rate, SR) das wichtigste Kriterium, gefolgt von der Pfadqualität. Der Grund dafür liegt darin, dass in ähnlichen POMDP-Benchmarks (wie Procgen, MiniGrid oder POPGym) die Erfolgsquote durchgehend als Standard gilt. Die grundsätzliche Fähigkeit, das Problem überhaupt zu lösen, wird in diesem Kontext höher gewichtet als eine perfekte Routenplanung.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{4.2cm} p{5cm} p{5.3cm}}
        ...
        \midrule
        \textbf{Catastrophic Forgetting} \newline (Abrupter Leistungsabfall) & Erfolgsquote stürzt beim Phasenwechsel drastisch ab und erholt sich nicht. & Duales Gate eingeführt: Phasenwechsel erst bei phasenspezifischer Ziel-Erfolgsquote ($85/70/70/60\,\%$) in zwei aufeinanderfolgenden Evaluationen oder bei Erschöpfung des Schrittbudgets. \\[2ex]
        ...
        \textbf{Reward Hacking} \newline (Pendeln zwischen Zellen) & Steigender Episoden-Reward bei stagnierender oder sinkender Erfolgsquote. & Distanzfeld-Puffer deutlich vergrößert; Potenzialfunktion auf mathematische Korrektheit geprüft. \\
        \bottomrule
    \end{tabular}
\end{table}
...
Dieses Kapitel beschreibt die praktische Implementierung der in Kapitel~\ref{chap:methodik} vorgestellten Konzepte, zunächst der Weltgenerierung (Abschnitt~\ref{sec:umsetzung_noise} bis~\ref{sec:umsetzung_movement_combat}), anschließend des RL-Trainings (Abschnitt~\ref{sec:impl_rl}). Die Weltgenerierung sowie die zugehörige Client-Anwendung sind in der Programmiersprache C++ realisiert, für die grafische Darstellung des Clients kommt die Bibliothek Raylib zum Einsatz. Das Hauptziel der Weltgenerierung besteht darin, eine performante, deterministische und speichereffiziente Erzeugung von Welt-Chunks zur Laufzeit zu gewährleisten.
...
\label{sec:umsetzung_stack}
Weltgenerierung, Simulationskern und Client sind vollständig in C++ umgesetzt. Tabelle~\ref{tab:world_stack} gibt einen kurzen Überblick über die verwendeten Werkzeuge.

\begin{table}[htbp]
    \centering
    \caption{Eingesetzte Werkzeuge der Weltgenerierung und des Clients}
    \label{tab:world_stack}
    \begin{tabular}{p{3.4cm}p{6.6cm}p{3.4cm}}
        \toprule
        \textbf{Werkzeug} & \textbf{Rolle im System} & \textbf{Variante} \\
        \midrule
        C++ & Implementierungssprache von Kern, Headless-Runner und Client & Standard 20 \\
        CMake & Build-System für alle Komponenten und Bindungen & $\ge 3.20$ \\
        Raylib & Grafik- und Eingabebibliothek des spielbaren Clients & Version 5.5 \\
        nlohmann/json & Einlesen der Konfigurationsdatei zur Laufzeit & Version 3.11.3 \\
        pybind11 & Brücke zwischen C++-Simulationskern und Python (siehe Abschnitt~\ref{sec:impl_rl_stack}) & Version 2.13.6 \\
        \bottomrule
    \end{tabular}
\end{table}

Der Simulationskern (\texttt{stoneforge\_core}) ist als eigenständige Bibliothek angelegt und wird von drei getrennten Zielen eingebunden: dem spielbaren, Raylib-basierten Client, einem Headless-Runner für Benchmarks (siehe Abschnitt~\ref{sec:ergebnisse_performance}) und dem über pybind11 erzeugten Python-Modul für das RL-Training. Alle drei greifen damit auf identischen Quellcode zurück, wodurch Divergenzen zwischen der im Client gespielten und der im Training verwendeten Weltlogik ausgeschlossen sind. Ein früherer SDL2-Client existiert im Quellcode als deaktivierte Rückfalloption, wird jedoch nicht mehr gepflegt.
...
\begin{itemize}
    ...
    \item \texttt{salt}: Ein variabler 64-Bit-Wert, der als Salt dient. Er verhindert, dass unterschiedliche Generierungsebenen (zum Beispiel Dichte, Erz- und Baumpopulation) auf derselben Koordinate synchrone Muster aufweisen.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Bäume (\texttt{TileType::Tree}) sprießen ausschließlich auf bisher unbesetzten Feldern (\texttt{tile == TileType::Empty}) unter Berücksichtigung von \texttt{treeThreshold}.
    \end{itemize}
\end{enumerate}
...
Um unabhängig von den pseudozufälligen Rauschwerten und den gewählten Biomschwellenwerten eine unbedingte Spielbarkeit (\textit{Playability}) zu garantieren, implementiert das System mehrere deterministische Sicherheits-Fallbacks. Die primäre Garantie entsteht dabei, wie in Abschnitt~\ref{sec:heuristiken} beschrieben, bereits vorgelagert bei der BFS-basierten Wahl des Ausgangs; die im Folgenden beschriebenen Mechanismen sichern zusätzlich die unmittelbare Bewegungsfreiheit um Spawn- und Zielpunkt und stehen als abschaltbarer Fallback für den Pfad selbst bereit.

\subsubsection{Implementierung des Manhattan-Carvers und der Freiradien}
In Listing~\ref{lst:manhattan_carver} ist der relevante Ausschnitt aus der Methodenreihenfolge in \texttt{reset()} sowie die vollständige Implementierung der Methode \texttt{carveGuaranteedPath} dargestellt, deren methodischer Entwurf bereits vorgestellt wurde (siehe Kapitel~3.1.2). Im ausgelieferten Konfigurationsstand ist dieser Fallback deaktiviert; der Carver dient hier der Vollständigkeit der Implementierungsbeschreibung.
...
\begin{enumerate}
    \item \textbf{Freiradien-Erzeugung (Quadratischer Bereich):} Sowohl um die Spawn-Koordinate \texttt{spawn\_} als auch um die Exit-Koordinate \texttt{exit\_} wird über verschachtelte Schleifen ein quadratischer Radius der Größe \texttt{cfg.spawnClearRadius} beziehungsweise \texttt{cfg.exitClearRadius} vollständig freigeräumt (\texttt{TileType::Empty}). Dies stellt sicher, dass der Spieler ohne Kollisionsprobleme starten kann.
    \item \textbf{Schalterbasierte Pfadfreischaltung (\texttt{forceGuaranteedPath}):} Die Pfadgenerierung wird rein konfigurationsbasiert über das Flag \texttt{cfg.forceGuaranteedPath} gesteuert, das im ausgelieferten Konfigurationsstand deaktiviert ist (siehe Abschnitt~\ref{sec:konfiguration}). Der Carver dient damit als abschaltbarer Fallback, nicht als primärer Lösbarkeitsmechanismus.
...
Die Erzeugung von Seen und deren optischer Übergangszone (Strände) erfordert eine Entkopplung der logischen Weltrepräsentation von der visuellen Darstellung. Während Gewässer als harte Kollisions- beziehungsweise Freiräume im Weltmodell definiert sind, entstehen Strände dynamisch über Nachbarschaftsauswertungen im Render-Pfad.

\subsubsection{Mathematische Seen-Maskierung (\texttt{lakeMaskAt})}
Gewässer dürfen im Gegensatz zu einzelnen Bäumen oder Hindernissen nicht als vereinzelte Pixel-Fehler erscheinen, sondern müssen als zusammenhängende, organische Binnengewässer im Terrain hervortreten. Dazu verwendet die Funktion \texttt{lakeMaskAt} niederfrequentes Rauschen (\textit{Low-Frequency Noise}).
...
Anstatt Sand/Strand als eigenständige Kacheltypen (\texttt{TileType}) im Speicher zu verwalten, wird die Strandzone zur Laufzeit im Render-Pfad des Clients anhand einer $3 \times 3$-Nachbarschaftsanalyse abgeleitet.
...
\begin{itemize}
    ...
    \item \textbf{Konditionale Überlagerung:} Die Funktion \texttt{lakeOrAdjacentAt} prüft, ob sich im direkten $3 \times 3$-Umfeld ein Gewässer befindet. Ist dies der Fall und handelt es sich nicht um eine blockierte Kachel (zum Beispiel Wand oder Baum), rendert die Engine dynamisch ein Sand-Overlay unter dem Basis-Boden.
\end{itemize}

\begin{figure}[htbp]
    \centering
    \includegraphics[height=6cm]{Bilder/lakes.PNG}
    \caption{Prozedural generierte Seenstruktur mit organischer Uferlinie und Strand-Overlay, eigene Darstellung (Screenshot des Prototyps).}
...
Abbildung~\ref{fig:biomes_grid} vergleicht vier der sieben implementierten Biome anhand von Screenshots des Prototyps. Die unterschiedlichen Untergrund-Farbpaletten sowie Objekt- und Hindernisdichten ergeben sich aus den biomspezifischen Schwellenwerten (\texttt{wallThreshold}, \texttt{oreThreshold}, \texttt{treeThreshold}) aus Abschnitt~\ref{sec:umsetzung_biome}.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[height=5.5cm]{Bilder/forest_biome.PNG}
...
Die Besiedlung der prozeduralen Spielwelt teilt sich architektonisch in zwei Subsysteme: statische, biomabhängige Geländestrukturen (Landmarken), die direkt in das Raster der Chunks integriert werden, sowie ein dynamisches, entitätsbasiertes Gegner-Spawnsystem (\textit{Lazy Mob Spawning}), das über die Simulationsschleife gesteuert wird.

\subsubsection{Statische Geländestrukturen in der Chunk-Generierung}
Strukturen wie Ruinen, Felsformationen oder spezielle Biomsymbole sind keine eigenständigen Dynamic-Entities, sondern werden als spezielle Kacheltypen (\texttt{TileType::StructureGrassland}, \texttt{TileType::StructureMountain} etc.) während der Erzeugung eines Chunks deterministisch eingebrannt.
...
Im Gegensatz zu Terrain-Objekten entstehen Gegner (\textit{Mobs}) nicht vorab bei der Chunk-Generierung, sondern werden lazily erzeugt, sobald der Spieler sich neuen Chunks nähert.
...
\begin{itemize}
    ...
    \item \textbf{Validierung und Biom-Adaption:} Ein Gegner wird nur auf passierbaren, unbesetzten Feldern außerhalb der Spielerposition platziert. Der Gegnertyp (\texttt{MobVariant}) richtet sich dabei über \texttt{mobVariantForBiomeTag} direkt nach den Eigenschaften des jeweiligen Bioms (zum Beispiel Wüsten- oder Eis-Varianten).
    \item \textbf{Begrenzte Verfolgungslogik:} Die KI der Gegner nutzt zwei Reichweiten (\texttt{detectRange} und \texttt{loseRange}). Ein Aggro-State wird nur getriggert, wenn der Spieler sich im Nahbereich (zum Beispiel in einem $7 \times 7$-Umfeld) befindet. Bewegungen erfolgen schrittweise und nur auf unbesetzte, begehbare Felder.
\end{itemize}

\subsection{Spieler-Bewegung, Kollisionsabfrage und Kampfdynamik}
\label{sec:umsetzung_movement_combat}

Die Interaktion des Spielers mit der prozedural erzeugten Spielwelt erfolgt über ein diskretes, grid-basiertes Koordinatensystem. Bewegung, Terrain-Restriktionen und Kampfhandlungen werden deterministisch innerhalb der Simulations-Engine ausgewertet, während spezifische Umgebungs-Effekte (wie Wasserverlangsamung) im Client-Render-Pfad moduliert werden.
...
Gewässer (\texttt{isLakeAt}) stellen im Weltmodell keine unpassierbare Barriere dar, sollen den Spieler jedoch beim Durchqueren spürbar ausbremsen. Anstatt die Core-Simulationslogik mit veränderlichen Geschwindigkeitszuständen zu verkomplizieren, setzt der Client im Render-Pfad eine Takt-Verlangsamung um:
...
Da diese Verlangsamung ausschließlich im Client umgesetzt ist, wirkt sie im headless betriebenen RL-Training nicht: Der Agent durchquert Seen dort ohne Zeitverlust, während ein menschlicher Spieler spürbar ausgebremst wird. Trainings- und Spielumgebung unterscheiden sich in diesem Punkt.

\subsubsection{Flächenbasiertes Kampfsystem und Proximity Damage}
Das Kampfsystem ist bewusst schlank strukturiert, um eine hohe Performanz und Vorhersehbarkeit im Runden- beziehungsweise Taktmodell zu gewährleisten.
...
\subsubsection{Sichtfeld und dynamische Beleuchtung im Client}
Zur Verstärkung des Entdeckungscharakters blendet die Render-Engine des Clients ein kreisförmiges Sichtfeld um die Spielfigur ein; Bereiche außerhalb des aktiven Sichtradius werden abgedunkelt dargestellt. Diese Verdunkelung ist rein clientseitig umgesetzt und wirkt sich nicht auf den Beobachtungsvektor des RL-Agenten aus, der unverändert ein quadratisches $15 \times 15$-Fenster liefert (siehe Abschnitt~\ref{sec:impl_rl_env}).
...
Verwendet wird \texttt{sb3-contrib} als direkte Ergänzung zum Stable-Baselines3-Kernframework. Der Grund dafür ist pragmatisch: Das Standard-Framework bringt von Haus aus keine rekurrente Policy mit, die aber für den LSTM-Ansatz (RecurrentPPO) benötigt wird. Obwohl das Framework auch Algorithmen wie DQN und A2C anbietet, konzentriert sich das Training ausschließlich auf den Vergleich von PPO und RecurrentPPO. Um den Durchsatz zu maximieren, laufen konstant 16 Instanzen der Umgebung parallel; die Anzahl folgt der in der Praxis gebräuchlichen Konvention, parallele Environment-Instanzen als Zweierpotenz zu wählen, ohne dass die exakte Kernzahl der verwendeten Hardware im Rahmen dieser Arbeit gezielt abgestimmt wurde.
...
\begin{itemize}
    ...
    \item \textbf{Deaktivierte Gegner:} Beim Reset der Umgebung stempelt der Wrapper die Weltgenerierung explizit mit \texttt{disable\_mobs=True}. Es werden daher während des Trainings keine Gegner erzeugt; der Agent kann also weder Schaden nehmen noch angreifen, obwohl der Reward-Term für Trefferschaden und die Trefferpunkte-Komponente der Beobachtung im Code erhalten bleiben.
    \item \textbf{Das wandernde Sichtfenster:} Der Agent sieht immer nur den Ausschnitt direkt um sich herum. Diese Einschränkung sorgt erst dafür, dass ein partiell beobachtbarer Entscheidungsprozess (POMDP) entsteht (siehe Kapitel~2.2).
\end{itemize}

Abbildung~\ref{fig:obs_schema} fasst diesen Aufbau schematisch zusammen.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{Bilder/observation_schema.png}
    \caption{Aufbau des 229-dimensionalen Beobachtungsvektors, Konzeptdarstellung, eigene Darstellung.}
    \label{fig:obs_schema}
\end{figure}
...
\begin{itemize}
    \item \textbf{\texttt{lstm\_hidden\_size} \& Architektur:} Das LSTM nutzt eine rekurrente Schicht mit 256 Einheiten (\texttt{lstm\_hidden\_size=256}), die nicht zwischen Actor und Critic geteilt wird (\texttt{shared\_lstm=False}). Das MLP-Netz nutzt hingegen zwei klassische Schichten à 256 Neuronen. Dies führt zu einer deutlichen Parameter-Asymmetrie: Das LSTM-Modell besitzt 1.038.917 Parameter, das MLP-Modell lediglich 250.629.
    \item \textbf{\texttt{batch\_size}:} Das MLP nutzt einen Wert von 256, weil die $256 \times 16$ gesammelten Schritte damit in exakt 16 Minibatches je Epoche zerfallen; der Wert liegt bewusst über dem Framework-Standard von 64. Das LSTM nutzt hingegen einen stark reduzierten Wert von 8, was bei denselben $4096$ gesammelten Schritten 512 Minibatches je Epoche ergibt. Dies ist eine technische Notwendigkeit für den LSTM-Critic: Die deutlich höhere Zahl an Gradientenschritten stabilisiert das Training, während ein größerer Batch von 64 den Critic spürbar destabilisierte (siehe Abschnitt~\ref{sec:eval_hyperparameter}).
    \item \textbf{\texttt{n\_epochs} \& \texttt{n\_steps}:} Identisch bei beiden Verfahren. Jede Aktualisierung basiert auf 256 gesammelten Zeitschritten je der 16 parallelen Umgebungen, insgesamt also 4096 Schritten, die über 10 Epochen (\texttt{n\_epochs=10}) trainiert werden.
    \item \textbf{Durchsatz:} Diese Architekturunterschiede spiegeln sich deutlich in der Rechengeschwindigkeit wider. Gemessen auf der in Abschnitt~\ref{sec:ergebnisse_performance} genannten Entwicklungsmaschine erreicht MLP im Training einen Durchsatz von $5\,989$ Schritten pro Sekunde, während das schwere LSTM-Netz bei $104$ Schritten pro Sekunde limitiert.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Das duale Gate:} Eine Phase endet, wenn \textit{entweder} die Ziel-Erfolgsquote (Target-SR) in zwei aufeinanderfolgenden Evaluationen erreicht wird, \textit{oder} das fest definierte Schrittbudget (Budget) aufgebraucht ist. In der Praxis zeigte sich, dass in Phase 3 und 4 stets das Schrittbudget den Phasenwechsel erzwang, da das Leistungs-Gate nicht mehr erreicht wurde. In den leichteren Phasen 1 und 2 setzte sich das Budget bei den LSTM-PPO-Läufen ebenfalls mehrheitlich durch: In Phase 1 erreichte nur einer der sieben Läufe das Leistungs-Gate vorzeitig, in Phase 2 gelang dies dreien, während die übrigen Läufe auch dort das jeweilige Schrittbudget vollständig ausschöpften. 
    \item \textbf{Asymmetrische Evaluation:} In Phase 3 und Phase 4 trainiert der Agent auf einer Distanz von 25 bis 45 Schritten, wird aber auf einem künstlich verengten, schwierigeren Fenster von 35 bis 45 Schritten evaluiert. Das Gate soll damit gezielt die schweren Fälle der Distanzspanne prüfen, statt durch die im Training enthaltenen kürzeren, leichteren Distanzen künstlich beschönigt zu werden.
...
Abbildung~\ref{fig:v12_lernkurven} zeigt den tatsächlichen Trainingsverlauf aller sieben LSTM-PPO-Läufe, getrennt nach Curriculum-Phase.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{Bilder/v12_lernkurven.png}
    \caption{Erfolgsquote der sieben v12-Läufe über die Trainingsschritte, je Curriculum-Phase, eigene Darstellung.}
    \label{fig:v12_lernkurven}
\end{figure}

Die Streuung der dünnen Einzellauf-Linien bleibt über alle vier Phasen hinweg beträchtlich, ein einzelner Lauf ist damit kein verlässlicher Indikator für das Verhalten des Verfahrens. In Phase 3 und 4 verharrt zudem auch das Ensemble-Mittel unterhalb der jeweiligen Ziel-Erfolgsquote, was den oben beschriebenen Befund bestätigt, dass in diesen Phasen durchgehend das Schrittbudget statt des Leistungs-Gates den Phasenwechsel auslöste.

Der Basiswert $0{,}05$ selbst geht auf einen frühen Vergleich zurück: Bei einem niedrigeren Wert von $0{,}01$ blieb die erklärte Varianz des Critics nahe null und die Erfolgsquote nach 50.000 Schritten bei $0\,\%$, während $0{,}05$ die erklärte Varianz auf über $0{,}9$ trieb und damit erst ein lernfähiges Value-Netz ermöglichte (siehe Abschnitt~\ref{sec:eval_hyperparameter}). Um dem Agenten den Übergang in schwerere Phasen zu erleichtern, wird der Entropie-Koeffizient (\texttt{ent\_coef}) ausgehend von diesem Basiswert in Phase 3 über 500.000 Schritte stetig auf $0{,}001$ abgesenkt (Annealing). In Phase 4 wird er zu Beginn der Phase zusätzlich um eine weitere Zehnerpotenz auf fix $0{,}0001$ abgesenkt, da diese Phase gezielt als \textit{Greedy Fine-Tune} konzipiert ist: Die Modellauswahl erfolgt hier anhand der deterministischen statt der stochastischen Erfolgsquote, wofür der Agent von einem explorativen nahezu vollständig in ein deterministisches Verhalten übergehen muss.

\subsection{Reward-Shaping und Distanzfeld}
\label{sec:impl_rl_reward}
Die Belohnungsstruktur ist überwiegend direkt in den performanten C++-Simulationskern eingewoben, mit Ausnahme einer im Python-Wrapper der Umgebung nachgelagerten Anti-Stuck-Strafe. Entgegen früheren Entwurfsstadien wurde die Strafen-Struktur stark entschlackt, da sich zeigte, dass sich aufsummierende Strafen (Penalty-Stacking) das Erkundungsverhalten des Agenten unterdrückten. 
...
Ein normaler Schritt kostet $-0{,}01$, das Betreten einer neuen Kachel bringt $+0{,}02$, Stehenbleiben wird mit $-0{,}04$ bestraft. Diese Stehenbleiben-Strafe (\texttt{idleAction}) greift ausschließlich bei den Aktionen \texttt{Wait} und \texttt{Noop} des Simulationskerns; im RL-Binding sind ausschließlich die vier Bewegungsaktionen zugelassen (siehe Abschnitt~\ref{sec:impl_rl_env}), sodass diese Strafe während Training und Evaluation nie ausgelöst wird und als Rest aus dem spielbaren Client im Reward-Code verbleibt. Wiederholt der Agent eine Zwei-Schritt-Schleife, erhält er $-0{,}05$ (ein Wert, der von ehemals $-0{,}15$ abgeschwächt wurde, da die Kombination aus einem damaligen, inzwischen aus dem Reward-Code entfernten eskalierenden Wand-Penalty und der scharfen Loop-Strafe das Erkunden in engen Korridoren teurer machte als bloßes Stillstehen; die Erkennung der Schleife blieb erhalten, nur ihre Strafhöhe wurde moderater gesetzt).

Nimmt er Schaden, kostet das $-0{,}5$ pro Schadenspunkt. Der Terminal-Reward für das Erreichen des Ziels beträgt $+100{,}0$, ein Timeout durch das Episodenlimit (4000 Schritte) resultiert in $-10{,}0$. Zusätzlich verfolgt der Python-Wrapper der Umgebung die Besuchshäufigkeit jeder Position: Wird ein Feld zum 26.\ Mal oder öfter besucht, zieht das $0{,}03 \cdot \min(\text{Besuche}/25,\ 2{,}0)$ vom Reward ab, gedeckelt bei $-0{,}06$ ab dem 50.\ Besuch. Eine frühere Fassung setzte diese Schwelle noch bei 300 Besuchen an; die Verschärfung auf 25 beziehungsweise 26 Besuche greift damit deutlich früher in wiederholtes Aufsuchen derselben Position ein, ohne dass die konkreten neuen Zahlenwerte über eine eigene Parameterstudie hinaus abgesichert wurden.
...
\begin{itemize}
    \item \textbf{Potenzialfunktion $\Phi$:} Definiert als $-\text{BFS-Distanz}(s) / 128{,}0$. Da $\gamma$ passend zum restlichen RL-Setup bei $0{,}999$ liegt und der Faktor $\beta$ auf $2{,}5$ skaliert ist, ergibt sich ein reines Shaping-Signal von $+0{,}0203$ je Feld Fortschritt (bei einer Distanz von 40), was inklusive der Schrittkosten von $-0{,}01$ einen Nettoeffekt von $+0{,}01$ ergibt. Der Diskontierungsfaktor $\gamma$ liegt damit deutlich über dem in der PPO-Literatur gebräuchlichen Standardwert von $0{,}99$: Bei den bis zu 4000 Schritte langen Episoden dieser Arbeit würde der niedrigere Standardwert das Terminal-Reward über den Episodenverlauf auf einen verschwindend kleinen Diskontfaktor in der Größenordnung von $10^{-18}$ drücken, während bei $\gamma = 0{,}999$ noch rund $0{,}018$ verbleiben. Der höhere Wert verhindert damit, dass der spärliche Erfolgs-Reward am Episodenende gegenüber den dichten PBRS-Zwischensignalen praktisch bedeutungslos wird.
    \item \textbf{Das Trainings-Distanzfeld (BFS):} Hier müssen zwei Systeme auseinandergehalten werden. Die nachträgliche Erreichbarkeitsprüfung bei der \textit{Weltgenerierung} ist deaktiviert (die Lösbarkeit wird dort bereits durch die BFS-basierte Ausgangsplatzierung garantiert, siehe Abschnitt~\ref{sec:umsetzung_pathing}). Das Distanzfeld für das \textit{Training} ist jedoch hochaktiv: Es wird bei jedem Episoden-Reset ausgehend vom Ausgang berechnet. Es dient sowohl dem Shaping als auch als Nenner für die spätere Wegverhältnis-Metrik.
...
Die Baselines aus Listing~\ref{lst:baselines} offenbaren ein wichtiges technisches Detail:

\begin{itemize}
    \item \textbf{BFS-Optimum:} Dient nur der Messung der absoluten Mindestschritte (Wegverhältnis) und wird nicht als Policy ausgeführt.
...
Dies macht die Kompass-Heuristik de facto \textbf{wandblind}. Sie nutzt keine Pfadfindungs-Logik (wie etwa den BFS-Gradienten) und kennt keine Hindernisse. Sie rennt stumpf gegen Wände, bis der Zufallsanteil $\varepsilon$ sie wieder löst. Dass ein komplett kartenblindes Verfahren in der Evaluation derart stark abschneidet, ist eine zentrale Erkenntnis für die spätere Auswertung und erklärt zugleich, warum bei dieser Baseline im getesteten Bereich $\varepsilon \in [0{,}3; 0{,}9]$ mit steigendem Zufallswert $\varepsilon$ die Erfolgsquote steigt und das Wegverhältnis zugleich sinkt. Dieser Zusammenhang gilt jedoch nicht uneingeschränkt: Bei $\varepsilon=1$ entspricht die Heuristik exakt dem Random-Agent, dessen Erfolgsquote von 5,2\,\% (siehe Abbildung~\ref{fig:eval_sr}) weit unter dem Wert bei $\varepsilon=0{,}9$ liegt, sodass die Kurve zwischen beiden Randwerten einbrechen muss.

\subsection{Vollständige Hyperparameter-Konfiguration}
\label{sec:impl_rl_parameter}
Um die vollständige Reproduzierbarkeit zu gewährleisten, sind in Tabelle~\ref{tab:parameter_training} alle verbleibenden, fest im Code verankerten Hyperparameter für den Optimierer (PPO) sowie die generellen Testbedingungen aufgeführt. Für die dort nicht gesondert diskutierten Optimierer-Parameter (\texttt{learning\_rate}, \texttt{gae\_lambda}, \texttt{clip\_range}, \texttt{vf\_coef}) werden die etablierten Standardwerte aus \citet{schulman2017proximal} beziehungsweise die Framework-Defaults übernommen, da die Trainingshistorie (siehe Tabelle~\ref{tab:hyperparameter_wirkung}) keinen Anlass zur Abweichung zeigte. Episoden werden regulär nach einem Limit von 4000 Schritten gestoppt. Zusätzlich erzwingt das System einen Early-Stop, wenn der Agent 256 Schritte lang keine positive Belohnung sammelt; auch dieser Schwellenwert folgt der Zweierpotenz-Konvention verwandter Zähl- und Batch-Parameter dieser Arbeit und wurde nicht durch eine eigene Ablationsstudie gegen benachbarte Werte abgesichert. Um Varianzen durch die Generierung auszugleichen, werden die Ergebnisse über sieben unterschiedliche Zufallssaaten (Seeds) gemittelt.
...
\begin{figure}[htbp]
    ...
    \caption{Generierte Spielwelt mit dynamischer Beleuchtung, Seenstrukturen und biomabhängigen Kacheltypen, eigene Darstellung (Screenshot des Prototyps, Debug-Overlay entfernt).}
...
\begin{itemize}
    ...
    \item \textbf{Organische Gewässer und Strand-Overlay:} Die durch niederfrequentes Rauschen erzeugten Seen treten als zusammenhängende, abgerundete Wasserflächen auf. Das im Render-Pfad verankerte Strand-Overlay erzeugt an den Begehbarkeitsgrenzen zuverlässig helle Sandstrukturen.
    ...
    \item \textbf{Landmarken und Strukturen:} Vereinzelt hervorgehobene symmetrische Fliesenmuster (zum Beispiel die quadratischen Rauten-Strukturen im oberen Zentrum) belegen das erfolgreiche Stempeln deterministischer Muster bei der Chunk-Initialisierung.
\end{itemize}

\subsection{Artefaktanalyse: Das Phänomen des Biome-Swallowing}
\label{sec:ergebnisse_swallowing}
Trotz der optisch ansprechenden, stufenlosen Farbübergänge durch die gewichtete Biom-Mischung (\texttt{biomeTintAtTile}) deckt die praktische Evaluation eine systematische Schwachstelle des gewählten \textit{Biome Blending}-Ansatzes auf.

Da die Gewichtung der Biomfarben über ein $3 \times 3$-Chunk-Umfeld berechnet wird (\texttt{biomeWeights}), besitzen flächenmäßig dominante Nachbarbiome eine überproportionale Strahlkraft auf dazwischenliegende, kleinere Biome. Dieser Effekt wird als \textbf{Biome-Swallowing} (Verschlucken von Kleinbiomen) bezeichnet:

\begin{quote}
\textit{Isolierte Phänomenologie:} Besitzt ein einzelner Chunk (zum Beispiel eine $8 \times 8$-Wüste mit orangener Grundpalette) eine räumliche Ausdehnung von nur einem Chunk ($1 \times 1$) und liegt unmittelbar zwischen zwei ausgedehnten Biomen wie einem dichten Wald und einer großen Steppe, so dominiert der mathematische Mittelwert der $3 \times 3$-Nachbarschaft. 
\end{quote}
...
Um dieses Artefakt in zukünftigen Iterationen zu vermeiden, ohne auf weiche Farbübergänge verzichten zu müssen, kommen zwei Ansätze infrage:
...
\begin{enumerate}
    ...
    \item \textbf{Nicht-lineare Interpolations-Gewichtung:} Das Gewicht des eigenen Chunks im Zentrum muss bei der Farberechnung deutlich höher gewichtet werden (zum Beispiel Faktor 0,6 für den eigenen Chunk und nur 0,4 verteilt auf die 8 Nachbarn), um die visuelle Eigenständigkeit kompakter Biome zu sichern.
\end{enumerate}

\subsection{Performance-Evaluation und Benchmarking}
\label{sec:ergebnisse_performance}

Zur empirischen Überprüfung der Laufzeit- und Speichereffizienz wurde eine automatisierte Benchmark-Routine im Headless-Runner (siehe Abschnitt~\ref{sec:umsetzung_stack}) implementiert. Über einen dedizierten CLI-Parameter (\texttt{-{}-benchmark-world}) führt das System eine isolierte Leistungsmessung durch, ohne vom Grafik-Rendering beeinträchtigt zu werden. Gemessen wurde auf derselben Entwicklungsmaschine wie das RL-Training, einem Apple M1 Pro (CPU). Tabelle~\ref{tab:performance_summary} fasst die ermittelten Kennzahlen zusammen.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|p{3.6cm}|p{4.0cm}|p{5.6cm}|}
        ...
        Simulationsschritt (\textit{Step Proxy}) & $N = 5000$ Ticks & $\varnothing\,0{,}0137\,\text{ms}$ (p95: $0{,}02\,\text{ms}$) \\ \hline
        Speicherbedarf (Sparse vs. Dicht) & 2048 geladene Chunks & $162\,\text{KiB}$ (\texttt{unordered\_map}) vs. $130\,\text{KiB}$ \\ \hline
        Determinismus-Abweichung & 100 Seeds / $518.400$ Tiles & $0{,}0000\,\%$ (0 Mismatches) \\ \hline
    \end{tabular}
\end{table}
...
Diese geringe Erzeugungsdauer bestätigt die Effizienz des mehrstufigen Noise-Ansatzes und der optimierten Tile-Zuweisung. Dank der Mikrosekunden-Latenz kann das System neue Chunks beim Betreten durch den Spieler erzeugen (\textit{Lazy Loading}), ohne dass Ruckler (\textit{Frame Drops}) im Render-Thread wahrnehmbar sind.
...
\begin{itemize}
    \item \textbf{Durchschnittliche Step-Dauer:} $0{,}0137\,\text{ms}$ ($\text{p95} = 0{,}02\,\text{ms}$).
...
Das System bietet damit reichlich Spielraum für die grafische Darstellung im Client und ermöglicht selbst auf leistungsschwacher Hardware eine flüssige Bildrate bei konstanten $60\,\text{FPS}$.

\subsubsection{Speichereffizienz der Hash-Map-Architektur}
Ein zentraler Architekturentwurf der Spielwelt ist die Speicherung der Chunks in einer \texttt{std::unordered\_map<uint64\_t, Chunk>}. Bei $2048$ aktiven Chunks (bei $8 \times 8$ Fliesen pro Chunk insgesamt $131\,072$ Fliesen) beläuft sich der Speicherverbrauch der Hash-Map inklusive Dynamic-Overhead auf ca. $162\,\text{KiB}$. 

Im Vergleich dazu würde ein zweidimensionales, dicht besetztes Array derselben maximalen Spannen etwa $130\,\text{KiB}$ belegen. Der geringfügige Speicher-Overhead von $\approx 32\,\text{KiB}$ für die Hash-Metadaten wird durch den Vorteil aufgewogen, dass die Welt theoretisch unendlich ausgedehnt sein kann, ohne unbesetzte Weltbereiche vorab im Arbeitsspeicher allozieren zu müssen.
...
Die Fehlerrate liegt exakt bei $0{,}00\,\%$. Dies beweist, dass sowohl das Noise-Cascading für Höhen und Biome als auch die BFS-basierte Ausgangsplatzierung vollständig deterministisch arbeiten und keine unkontrollierten Nebeneffekte oder Zustandsartefakte vorliegen.
...
\begin{itemize}
    ...
    \item \textbf{Garantierte Pfad-Topologie:} Die mathematisch strikte, BFS-basierte Platzierung des Ausgangs garantiert zu $100\,\%$, dass zwischen Startpunkt (\texttt{spawn\_}) und Ziel (\texttt{exit\_}) stets ein begehbarer Pfad existiert. Unlösbare Welt-Instanzen werden dadurch deterministisch ausgeschlossen.
    \item \textbf{Sparsamer Speicherbedarf bei bedarfsgesteuertem Laden:} Dank der spärlich besetzten Hash-Map-Architektur (\texttt{std::unordered\_map}) reserviert die Welt Speicher ausschließlich für tatsächlich generierte Chunks, anstatt riesige 2D-Arrays vorab im RAM zu reservieren; bei 2048 geladenen Chunks liegt der gemessene Bedarf bei $\approx 162\,\text{KiB}$ (siehe Tabelle~\ref{tab:performance_summary}).
\end{itemize}

\subsubsection{Identifizierte Schwachstellen und Limitationen}
\begin{itemize}
    \item \textbf{Geometrische Künstlichkeit des Manhattan-Carvers:} Da der Pfad-Einkerber Kacheln entlang rechtwinkliger Koordinatendifferenzen aufbricht, neigt die Logik zur Bildung auffälliger „L-Shapes“ (rechtwinklige Gänge). In offenem Terrain (zum Beispiel Wiese oder Wüste) wirken diese harten $90^\circ$-Achsenbrüche stellenweise unnatürlich und brechen die organische Anmutung der Noise-Generierung.
    \item \textbf{Beschränkung auf flaches 2D-Raster:} Das System arbeitet auf einem rein zweidimensionalen Gitter. Echte Höhenunterschiede, Plateaus, Höhlensysteme auf mehreren Ebenen oder Verdeckungslogiken (zum Beispiel Rampen oder Brücken) können ohne Erweiterung um ein $Z$-Achsengitter oder Iso-Layer nicht abgebildet werden.
    \item \textbf{Unbegrenztes RAM-Wachstum ohne Chunk-Unloading:} Obwohl der Einstiegsspeicherbedarf minimal ist, besitzt der aktuelle Chunk-Cache keinen Entlademechanismus (zum Beispiel \textit{Least Recently Used} / LRU-Unloading). Besucht ein Spieler in einer extrem langen Sitzung hunderttausende Chunks, wächst die \texttt{unordered\_map} kontinuierlich an.
    \item \textbf{Farb-Absorption bei Kleinbiomen (\textit{Biome-Swallowing}):} Wie in Abschnitt~\ref{sec:ergebnisse_swallowing} analysiert, führt das räumlich gewichtete \textit{Biome Blending} über $3 \times 3$ Chunks dazu, dass isolierte Einzel-Chunks ($1 \times 1$) farblich von ihren dominanten Nachbarbiomen überstrahlt werden.
\end{itemize}

\section{Evaluation des RL-Trainings}
\label{sec:evalRL}

Dieser Abschnitt evaluiert die trainierten RL-Verfahren hinsichtlich Erfolgsquote, Wegverhältnis und Robustheit gegenüber den ungelernten Referenzpunkten aus Abschnitt~\ref{sec:methodik_baselines}. Datengrundlage sind sieben unabhängige Trainingsläufe je Modell, ausgewertet auf dem in Abschnitt~\ref{sec:methodik_protokoll} definierten Testset A.

\subsection{Erfolgsquote der trainierten Verfahren}
\label{sec:eval_sr}
Die Erfolgsquote ist die zentrale Metrik zur Beurteilung, ob ein Verfahren die Navigationsaufgabe überhaupt löst. Abbildung~\ref{fig:eval_sr} stellt die beiden trainierten Modelle den ungelernten Referenzpunkten gegenüber; das LSTM-PPO-Modell erscheint dabei zusätzlich in der deterministischen Auswertung.
...
Das LSTM-PPO-Modell erreicht in der stochastischen Auswertung eine mittlere Erfolgsquote von 64,6\,\% auf Testset A. Über die sieben Durchläufe hinweg liegt die Standardabweichung bei 10,1 Prozentpunkten, die Einzelwerte schwanken zwischen 51,6\,\% und 76,8\,\%. Auf Holdout B fällt das Ergebnis mit 68,8\,\% $\pm$ 15,5 höher aus, was für eine gute Generalisierung auf ungesehene Seeds spricht. Im deterministischen Modus bricht die Leistung massiv ein: Die Quote fällt auf magere 29,1\,\% $\pm$ 8,0 auf Testset A und 32,6\,\% $\pm$ 12,7 auf Holdout B. Diese Werte stammen aus einer eigens auf denselben Testset-Seeds durchgeführten, einmaligen deterministischen Zusatzauswertung; das kanonische Eval-Skript reproduziert diesen Modus allerdings nicht mehr, weshalb die Ergebnisse hier nachrichtlich referenziert werden.\footnote{Der kanonische \texttt{-{}-models}-Sweep in \texttt{scripts/eval\_baselines.py} instanziiert Modelle aktuell ausschließlich mit \texttt{deterministic=False}; ein eigener CLI-Schalter für den deterministischen Modus existiert im aktuellen Skriptstand nicht mehr. Die hier berichteten Werte stammen aus einem früheren, gesondert durchgeführten Lauf; die vollständige Mess- und Änderungshistorie dazu ist in \texttt{CHANGELOG.md} dokumentiert.}

Beim MLP-PPO-Verfahren fällt das Bild schwächer aus als beim LSTM. Unter dem identischen Standardprotokoll (siehe Abschnitt~\ref{sec:methodik_protokoll}) erreicht das Modell stochastisch ausgewertet auf Testset A 33,5\,\% $\pm$ 25,5 und auf Holdout B 35,8\,\% $\pm$ 30,5. Die Streuung zwischen den sieben Seeds fällt damit mehr als doppelt so hoch aus wie beim LSTM, die Einzelwerte auf Testset A reichen von 4,0\,\% bis 65,2\,\%. Eine deterministische Zusatzmessung auf den Testset-Seeds liegt für das MLP-PPO-Modell nicht vor; der deterministische Modus wird daher nur für das LSTM-PPO-Modell berichtet (siehe Abschnitt~\ref{sec:methodik_protokoll}).

Der Systemvergleich fällt damit eindeutig aus: Die rekurrente LSTM-Architektur schlägt das gedächtnislose MLP-Netz deutlich. Der Vorsprung beträgt rund 31 Prozentpunkte, auch wenn die starken Schwankungen des MLP-Modells für Überschneidungen einzelner Läufe sorgen. Das LSTM bleibt dennoch das klar überlegene Verfahren, wobei Abschnitt~\ref{sec:eval_hyperparameter} diesen Vorsprung noch einordnet.

Bei nur sieben Läufen je Architektur und einer Standardabweichung von 25,5 Prozentpunkten beim MLP-Modell stellt sich die Frage, ob dieser Unterschied statistisch belastbar ist oder in der Streuung der kleinen Stichprobe verschwindet. Die zugrunde liegenden sieben Einzelwerte je Verfahren und Testset sind in Anhang~C aufgeführt. Ein Welch-Test auf die sieben Erfolgsquoten je Verfahren (Testset A, stochastisch) weist den Unterschied als statistisch signifikant aus ($t(7{,}8) = 2{,}99$, $p = 0{,}018$); ein verteilungsfreier Mann-Whitney-U-Test verfehlt die 5-Prozent-Schwelle dagegen knapp ($U = 40$, $p = 0{,}053$). Die Effektstärke fällt mit Cohens $d = 1{,}6$ groß aus.

Auf Holdout B liegen beide Tests im statistisch signifikanten Bereich (Welch: $p = 0{,}031$; Mann-Whitney: $U = 41$, $p = 0{,}038$). Die einzelnen 95\,\%-Konfidenzintervalle auf Testset A überschneiden sich zwar teilweise (LSTM: $[55{,}3; 73{,}9]$, MLP: $[9{,}9; 57{,}1]$), was bei einem Vergleich zweier einzelner Intervalle leicht als Beleg gegen Signifikanz gelesen werden könnte; das Konfidenzintervall der Differenz selbst schließt die Null jedoch nicht ein ($[7{,}0; 55{,}0]$). Der gemessene Vorsprung des LSTM ist damit trotz der geringen Stichprobengröße nach gängigen Kriterien statistisch abgesichert; einzig der verteilungsfreie Test auf Testset A bleibt knapp oberhalb der 5-Prozent-Schwelle.

\subsection{Wegverhältnis derselben Verfahren}
\label{sec:eval_eff}
Nur am Ausgang anzukommen, reicht als Bewertungskriterium nicht aus, wichtig ist auch der Weg dorthin. Abbildung~\ref{fig:eval_eff} zeigt daher das Wegverhältnis. Es wird pro erfolgreicher Episode als Verhältnis zwischen dem per Breitensuche (siehe Abschnitt~\ref{sec:grundlagen_bfs}) ermittelten kürzesten Weg dieser Episode und den vom Agenten tatsächlich gelaufenen Schritten berechnet und anschließend über alle erfolgreichen Episoden gemittelt. Da dieser Mittelwert der Verhältnisse nicht dem Verhältnis der Mittelwerte entspricht, lässt sich das berichtete Wegverhältnis nicht durch Division der weiter unten genannten Durchschnittswerte für BFS-Optimum und Schrittzahl nachrechnen. Ein Wert von 1,0 markiert das Optimum.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{Bilder/eval_pfadeffizienz.png}
    \caption{Evaluation -- Wegverhältnis der Verfahren in derselben Achsenordnung, eigene Darstellung.}
    \label{fig:eval_eff}
\end{figure}

Das BFS-Optimum liegt auf dem verwendeten Testset im Schnitt bei 40,1 Schritten. Das trainierte LSTM-Modell benötigt für eine erfolgreiche Episode dagegen durchschnittlich 1425 Schritte, bei einem gemittelten Wegverhältnis von lediglich 0,053. Der Agent läuft damit im Schnitt einen Weg, der mehr als 35-mal so lang ist wie nötig. Abbildung~\ref{fig:pfad_vs_bfs} macht diesen Unterschied an einer einzelnen Episode aus Testset A sichtbar.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{Bilder/agent_pfad_vs_bfs.png}
    \caption{Evaluation -- gelaufener Agentenpfad gegen BFS-Optimum auf Seed 7000 (Testset A), eigene Darstellung.}
    \label{fig:pfad_vs_bfs}
\end{figure}

Der abgebildete Lauf erreicht mit 456 gegenüber 43 BFS-optimalen Schritten ein Wegverhältnis von 0,094 und liegt damit über dem berichteten Mittel von 0,053; die wiederholten Schleifen im eingezeichneten Pfad zeigen aber bereits an dieser einzelnen, vergleichsweise günstigen Episode das charakteristische Herumirren, das das niedrige mittlere Wegverhältnis erklärt. Für das MLP-Modell liegt ein vergleichbarer Wert vor: Bei durchschnittlich 1084 Schritten je erfolgreicher Episode liegt das gemittelte Wegverhältnis bei 0,067 und damit höher als beim LSTM. Dieser Befund ist mit Vorsicht zu lesen, da die Kennzahl nur über die erfolgreichen Episoden gemittelt wird, wie der folgende Absatz zum Selektionseffekt zeigt: Bei einer Erfolgsquote von nur 33,5\,\% löst das MLP-Modell bevorzugt die leichten Seeds, was das Wegverhältnis künstlich anhebt.

Aufschlussreich wird der Blick auf die Referenzverfahren aus Abschnitt~\ref{sec:methodik_baselines}, deren vollständige Ergebnistabelle in Anhang~C aufgeführt ist. Der vollständig zufällig agierende Random-Agent benötigt in seinen seltenen erfolgreichen Episoden im Schnitt 2251,9 Schritte, bei einem gemittelten Wegverhältnis von lediglich 0,021. Der ungelernte Kompass-Agent mit einem Zufallsanteil von $\varepsilon=0{,}3$ erreicht ein Wegverhältnis von 0,177 und hat damit ein rund 3,3-mal so hohes Wegverhältnis wie das trainierte LSTM-Netz, auch wenn er dabei eine rund 13 Prozentpunkte niedrigere Erfolgsquote in Kauf nimmt. Das überraschendere Ergebnis liefert der $\varepsilon=0{,}8$-Kompass: Er übertrifft das trainierte Modell nicht nur bei der Erfolgsquote, sondern mit einem Wegverhältnis von 0,094 gleichzeitig auch bei der Weglänge.

Ein leichter Selektionseffekt ist hierbei zu berücksichtigen: In das Wegverhältnis fließen nur jene Episoden ein, bei denen der Ausgang gefunden wurde. Verfahren, die oft scheitern, wobei die gescheiterten Versuche meist sehr lange Irrwege sind, stehen dadurch statistisch etwas besser da, als sie eigentlich sind. Das erklärt den starken Kompass jedoch nicht vollständig. Die bittere Erkenntnis bleibt: Das Training führt zwar ans Ziel, aber alles andere als auf direktem Weg.

Ein plausibler Mitverursacher liegt in der Reward-Struktur selbst: Der Neuland-Bonus von $+0{,}02$ übersteigt die Schrittkosten von $-0{,}01$ (siehe Abschnitt~\ref{sec:impl_rl_reward}), sodass das Betreten einer noch unbesuchten Kachel unabhängig von ihrer Richtung zum Ziel netto positiv entlohnt wird. Dieser Anreiz zum flächigen Erkunden könnte das stark unterdurchschnittliche Wegverhältnis des LSTM mit erklären, ist im Rahmen dieser Arbeit jedoch nicht durch eine eigene Ablationsstudie belegt. Ein einzelner Retraining-Lauf mit einem auf unter $0{,}01$ gesenkten Neuland-Bonus, ausgewertet auf denselben Testset-Seeds, hätte diese Hypothese direkt und mit vertretbarem Aufwand testen können; dieser gezielte Eingriff bleibt als konkret benannter nächster Schritt offen.

\subsection{Die Determinismus-Lücke}
\label{sec:eval_determinismus}
Das LSTM-PPO-Modell wurde in zwei Modi ausgewertet: deterministisch, wobei stets die wahrscheinlichste Aktion gewählt wird, und stochastisch, wobei die Aktion aus der Wahrscheinlichkeitsverteilung gezogen wird. Der Unterschied zwischen beiden Auswertungen ist enorm, wie Abbildung~\ref{fig:eval_det} für jeden einzelnen der sieben Läufe zeigt.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{Bilder/eval_det_stoch.png}
    \caption{Evaluation -- Erfolgsquote der sieben LSTM-PPO-Läufe in deterministischer und stochastischer Auswertung, eigene Darstellung.}
    \label{fig:eval_det}
\end{figure}

Zwischen den beiden Modi klafft eine Lücke von durchschnittlich 35,5 Prozentpunkten (64,6\,\% gegenüber 29,1\,\%, siehe Abschnitt~\ref{sec:eval_sr}). Dieses Phänomen tritt konsequent in jedem der sieben Läufe auf: Kein einziger Lauf schneidet deterministisch besser ab als stochastisch.
...
Eine ergänzende Trajektorienanalyse stützt diese Vermutung, ohne sie vollständig zu bestätigen. Für zwanzig Seeds aus Testset A (ein einzelner Trainingslauf, Seed 1, Distanz 35--45, Schrittlimit 1500 statt der vollen 4000, aus Zeitgründen) richtet sich die deterministische Politik nur in 16,2\,\% der Schritte nach der BFS-optimalen Richtung, die stochastische in 23,4\,\%. Von den 13 gescheiterten deterministischen Episoden erschöpfen 9 das volle Schrittlimit, ohne durch einen erkennbaren Abbruch früher zu enden; in den übrigen 4 Episoden griff der in Abschnitt~\ref{sec:impl_rl_parameter} beschriebene Early-Stop nach 256 Schritten ohne positiven Reward. Bei der Mehrheit der gescheiterten Episoden greift dieser Mechanismus jedoch nicht: Da das PBRS-Shaping bei jeder Verringerung der BFS-Distanz einen positiven Einzelreward liefert, genügt bereits gelegentlicher, unsystematischer Fortschritt, um den Zähler immer wieder zurückzusetzen. Der Agent bewegt sich bis zum Limit weiter, ohne dem Ziel näherzukommen.

Einzelne Positionen werden dabei extrem häufig erneut besucht, in einem Fall 161-mal innerhalb von 1500 Schritten. Überraschend liegt die durchschnittliche Zahl mehrfach besuchter Positionen jedoch beim stochastischen Modus höher (90,3 gegenüber 58,5): Der Zufallsanteil erzeugt viele kurze Vor-und-Zurück-Bewegungen, während die deterministische Politik seltener, dafür aber in einzelnen Positionen deutlich hartnäckiger feststeckt. Die einfache Erklärung \glqq Zufall befreit aus der Sackgasse\grqq{} trifft den Befund damit nur teilweise; robuster ist die geringere Optimalausrichtung der deterministischen Politik.

\subsection{Einordnung gegenüber den Referenzpunkten}
\label{sec:eval_einordnung}
Um das Geleistete ins Verhältnis zu setzen, trägt Abbildung~\ref{fig:eval_tradeoff} die beiden Hauptmetriken, Erfolgsquote und Wegverhältnis, gegeneinander auf.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{Bilder/eval_zielkonflikt.png}
    \caption{Evaluation -- Erfolgsquote gegen Wegverhältnis für alle Verfahren, eigene Darstellung.}
    \label{fig:eval_tradeoff}
\end{figure}

Die fünf Varianten der Kompass-Heuristik aus Abschnitt~\ref{sec:methodik_baselines} ziehen im Diagramm eine zusammenhängende Kurve: Im getesteten Bereich steigt mit höherem Zufallsanteil $\varepsilon$ die Erfolgsquote, während das Wegverhältnis sinkt. Bei $\varepsilon=0{,}9$ steigt die Erfolgsquote noch einmal auf 92,0\,\%, das Wegverhältnis sinkt dabei jedoch auf 0,047 und bleibt damit unter dem LSTM-Wert von 0,053. Auffällig ist die Position des trainierten Modells: Es liegt vollständig unterhalb dieser Heuristik-Kurve.

Besonders hart fällt der Vergleich mit der Kompass-Kurve insgesamt aus. Von den fünf getesteten Varianten schlagen $\varepsilon=0{,}5$, $0{,}6$ und $0{,}8$ das LSTM-Netzwerk in \emph{beiden} Metriken gleichzeitig: bei $\varepsilon=0{,}8$ mit der größten Differenz, einer Erfolgsquote von 88,8\,\% gegenüber 64,6\,\% und einem Wegverhältnis von 0,094 gegenüber 0,053. Nur bei $\varepsilon=0{,}9$ reicht es, wie in Abbildung~\ref{fig:eval_tradeoff} zu sehen, wegen des knapp unterschrittenen Wegverhältnisses nicht zur Dominanz in beiden Metriken (siehe Abschnitt~\ref{sec:eval_eff}). Das wiegt umso schwerer angesichts dessen, was der Kompass eigentlich ist: eine primitive Heuristik, die nur Luftlinien-Richtungen kennt, Wände ignoriert und stumpf gegen Hindernisse rennt, bis der Zufall sie befreit.

Dass ein kartenblindes Verfahren ohne Trainingsphase zuverlässiger und direkter ins Ziel findet als ein Agent, der über bis zu 2,2 Millionen Zeitschritte hinweg optimiert wurde, ist die zentrale Pointe dieser Auswertung. Das ändert nichts daran, dass das LSTM besser lernt als das MLP. Für die Beurteilung, wie leistungsfähig Reinforcement Learning für dieses spezifische Problem tatsächlich ist, bleibt es jedoch ein starker Dämpfer.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{2.9cm}p{2.4cm}p{7.9cm}}
        ...
        \texttt{batch\_size} & $8$ (LSTM) & Ein Wert von $64$ destabilisierte den Critic spürbar (84\,\% Erfolgsquote bei $8$ gegenüber 48\,\% bei $64$, jeweils bei 150.000 Schritten). \\
...
\subsection*{Teilfrage 1: Aufbau, Performanz und Lösbarkeit der Spielwelt}
Die erste Teilfrage betraf den Aufbau einer performanten, seedbasierten und lösbaren Spielwelt. Die Antwort lässt sich an drei Punkten festmachen:

\begin{enumerate}
    \item \textbf{Strukturelle Lösbarkeit statt Parameter-Testen:}
    Die Lösbarkeit der erzeugten Instanzen hängt nicht an einem feingetunten, fehleranfälligen Parameterfenster, sondern an einer harten strukturellen Garantie. Bei jeder Weltinitialisierung wird der Ausgang ausschließlich an Positionen platziert, die eine Breitensuche (BFS) vom Startpunkt aus als erreichbar bestätigt. Zusätzlich steht der achsparallele Manhattan-Carver als abschaltbarer Fallback bereit (siehe Abschnitt~\ref{sec:umsetzung_pathing}). Unlösbare Weltinstanzen sind dadurch ausgeschlossen, egal wie dicht die restliche Wandstruktur generiert wird.

    \item \textbf{Abwechslung durch biomabhängige Parameter:}
    Die Konfigurationsparameter (wie die biomspezifischen Wanddichten zwischen $7\,\%$ und $25\,\%$) steuern demnach nicht das Ob der Lösbarkeit, sondern die Topologie: Sie bestimmen, wie umwegig und variantenreich die Pfade abseits der Hauptroute ausfallen, und erzeugen so die gewünschte visuelle und mechanische Varianz zwischen den Biomen.

    \item \textbf{Performanz und Determinismus als Trainingsgrundlage:}
    Mit einer durchschnittlichen Generierungsdauer von $13{,}12\,\mu\text{s}$ pro Chunk, einem Durchsatz von rund $73.000$ Simulationsschritten pro Sekunde (siehe Tabelle~\ref{tab:performance_summary}) und einer gemessenen Determinismus-Abweichung von exakt $0{,}0000\,\%$ (siehe Abschnitt~\ref{sec:ergebnisse_performance}) erfüllt die Weltarchitektur die Anforderungen, die ein paralleles RL-Training an seine Umgebung stellt: reproduzierbare Welten über Seeds und genug Durchsatz, damit nicht die Simulation zum Engpass wird.
\end{enumerate}

Damit ist die erste Teilfrage beantwortet: Eine faire und lösbare Welt entsteht nicht durch vorsichtiges Austarieren der Generierungsparameter, sondern durch die strukturelle Garantie bei der Platzierung des Ausgangs. Die Parameter formen die Welt, die Breitensuche sichert sie ab.

\subsection*{Teilfrage 2: Wahl des Trainingsverfahrens}
Die zweite Teilfrage suchte die Kombination aus Policy-Architektur und Trainingsverfahren, die die höchste Erfolgsquote und die besten Wege zum Ausgang erreicht. Bei der Erfolgsquote fällt die Antwort klar aus: Es ist das rekurrente LSTM-PPO-Netz, trainiert über das leistungsbasierte Vier-Phasen-Curriculum mit kleinen Batches und hohem Entropie-Koeffizienten (siehe Abschnitt~\ref{sec:methodik_curriculum}). Diese Kombination kommt stochastisch ausgewertet im Mittel auf 64,6\,\% ($\pm$ 10,1), das MLP-PPO-Modell unter identischem Curriculum nur auf 33,5\,\% ($\pm$ 25,5) (siehe Abbildung~\ref{fig:eval_sr}); der Unterschied ist signifikant ($p = 0{,}018$, siehe Abschnitt~\ref{sec:eval_sr}). Mit der partiellen Beobachtbarkeit kommt das LSTM-Netz im Mittel deutlich besser zurecht; zwar erreichen die besten MLP-Läufe das Niveau schwächerer LSTM-Läufe, am Gesamtbild ändert diese Überschneidung jedoch nichts.

Einen Dämpfer setzt die Determinismus-Lücke: Ohne Zufallsanteil fällt das LSTM-PPO-Modell von 64,6\,\% auf 29,1\,\% zurück, und zwar in jedem einzelnen der sieben Läufe (siehe Abbildung~\ref{fig:eval_det}). Daran hat auch die vierte Curriculum-Phase nichts geändert, deren Greedy Fine-Tuning genau diese Lücke schließen sollte (siehe Abschnitt~\ref{sec:methodik_curriculum}). Wie in Abschnitt~\ref{sec:eval_hyperparameter} eingeordnet, ist der Vorsprung gegenüber dem MLP zudem nicht sauber von der reinen Netzkapazität zu trennen, da das LSTM gut viermal so viele Parameter besitzt wie das MLP.

Die zweite Hälfte der Frage, die nach den besten Wegen zum Ausgang, muss dagegen negativ beantwortet werden: Keine der trainierten Kombinationen findet gute Wege (siehe Abbildung~\ref{fig:eval_eff}). Für eine erfolgreiche Episode braucht das beste trainierte Modell im Schnitt 1425 Schritte, das rechnerische Optimum liegt bei 40,1 Schritten. Das gemittelte Wegverhältnis von 0,053 wird selbst von trivialen Referenzheuristiken um mehr als das Dreifache übertroffen. Die gesuchte Kombination gewinnt also die Erfolgsquote, die besten Wege findet sie nicht.

Auf der Seite des Trainingsverfahrens dominieren wenige Einstellungen das gesamte Training (siehe Abschnitt~\ref{sec:eval_hyperparameter}). Der Entropie-Koeffizient entscheidet darüber, ob der Agent das Ziel überhaupt jemals sieht: Ein zu niedriger Wert lässt ihn das Navigieren komplett verlernen. Ausschlaggebend waren durchgehend Exploration und Gradientenstabilität, nicht die Kapazität oder Tiefe der neuronalen Netze.

\section{Fazit}
\label{sec:fazit_gesamt}

\subsection{Beitrag der Weltgenerierung}
\label{sec:fazit_weltgenerierung}
Im Rahmen dieser Arbeit wurde ein vollständiges, performantes System zur prozeduralen Generierung unendlicher 2D-Spielwelten auf Basis von C++ und Raylib entworfen und implementiert.

\begin{itemize}
    \item \textbf{Synthese aus Rauschen und Pfadgarantie:} Durch die Kombination mehrstufiger \textit{Value-Noise}-Gitter mit der BFS-basierten Ausgangsplatzierung konnte der Zielkonflikt zwischen organisch wirkenden Landschaften und deterministisch garantierter Begehbarkeit gelöst werden. Unlösbare Welt-Instanzen werden systematisch ausgeschlossen.
    \item \textbf{Hohe Speicher- und Laufzeiteffizienz:} Die Architektur nutzt eine bedarfsgesteuerte Chunk-Verwaltung (\textit{Lazy Loading}) mit einer $8 \times 8$-Kachelstruktur in Kombination mit einer spärlich besetzten Hash-Map-Struktur. Die Benchmarks bestätigen eine Chunk-Generierungszeit von durchschnittlich $13{,}12\,\mu\text{s}$ und einen Speicherbedarf von ca. $162\,\text{KiB}$ bei 2048 geladenen Chunks.
...
Die gestellten Anforderungen an Verlässlichkeit und Performanz wurden damit erfüllt; bei der Ästhetik bleiben die in Abschnitt~\ref{sec:ergebnisse_diskussion} benannten Artefakte, das Biome-Swallowing und die rechtwinkligen L-Shapes des Manhattan-Carvers, bestehen.

\subsection{Beitrag des RL-Trainings}
\label{sec:fazit_rl}
Im Rahmen dieser Arbeit wurde eine vollständige Trainings- und Evaluationspipeline für das Navigationsproblem auf der in Abschnitt~\ref{sec:fazit_weltgenerierung} beschriebenen Umgebung aufgebaut.

\begin{itemize}
    \item \textbf{Funktionierendes Curriculum mit dualem Gate:} Sieben unabhängige LSTM-PPO- und sieben unabhängige MLP-PPO-Läufe wurden über bis zu $2{,}2$ Millionen Zeitschritte hinweg leistungs- und budgetbasiert trainiert und unter einem einheitlichen, reproduzierbaren Protokoll ausgewertet (siehe Abschnitt~\ref{sec:methodik_protokoll}).
    \item \textbf{Rekurrenz-Vorteil statistisch abgesichert:} LSTM-PPO übertrifft MLP-PPO in der Erfolgsquote statistisch signifikant (Welch-Test, Mann-Whitney-U-Test, Cohens $d = 1{,}6$), auch wenn sich dieser Vorsprung nicht sauber von der höheren Parameterzahl des LSTM trennen lässt (siehe Abschnitt~\ref{sec:eval_hyperparameter}).
    \item \textbf{Determinismus-Lücke identifiziert:} Das LSTM-PPO-Modell verliert im deterministischen Modus einen Großteil seiner Erfolgsquote und fällt von 64,6\,\% auf 29,1\,\%. Der gelernte Vorteil beruht damit teilweise auf stochastischem Sampling und nicht allein auf einer verlässlichen Route (siehe Abschnitt~\ref{sec:eval_determinismus}).
    \item \textbf{Baseline-Vergleich als methodischer Befund:} Die einfache $\varepsilon$-Kompass-Heuristik übertrifft beide trainierten Modelle bei $\varepsilon=0{,}8$ in Erfolgsquote und Wegverhältnis gleichzeitig, was die Grenzen des gewählten RL-Ansatzes auf dieser konkreten Aufgabenstellung offenlegt (siehe Abschnitt~\ref{sec:eval_einordnung}).
\end{itemize}

Abbildung~\ref{fig:v12_zielkriterium} stellt das LSTM-PPO-Endergebnis den vorab festgelegten Zielkriterien von mindestens 70\,\% auf Testset A und mindestens 60\,\% auf Holdout B gegenüber, deren Zahlenwerte aus den in Abschnitt~\ref{sec:impl_rl_pipeline} beschriebenen Ziel-Erfolgsquoten der Phasen 3 und 4 übernommen wurden. Dort galten sie als deterministische Gate-Kriterien auf den Validierungs-Seeds; hier werden sie als nominale Schwellen auf die stochastische Hauptmetrik aus Abschnitt~\ref{sec:methodik_protokoll} angelegt, unabhängig von der ursprünglichen Gate-Modalität der Phasen.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{Bilder/v12_zielkriterium.png}
    \caption{Erfolgsquote des LSTM-PPO-Modells (n=7) gegen das Zielkriterium, Testset A und Holdout B, eigene Darstellung.}
    \label{fig:v12_zielkriterium}
\end{figure}

Das Kriterium für Testset A ($\geq 70\,\%$) wird mit 64,6\,\% verfehlt, während das Kriterium für Holdout B ($\geq 60\,\%$) mit 68,8\,\% erreicht wird; die eingezeichneten Streupunkte der sieben Einzelläufe machen zugleich sichtbar, dass beide Mittelwerte auf einer erheblichen Schwankungsbreite beruhen. Die zusätzlich gezeigten deterministischen Werte fallen deutlich niedriger aus, unterliegen jedoch keinem eigenen Zielkriterium.

Die Zielsetzung, verschiedene RL-Verfahren auf der Navigationsaufgabe zu trainieren und ihre Leistungsfähigkeit gegen ungelernte Referenzen einzuordnen, wurde damit erreicht. Ob Reinforcement Learning der geeignete Ansatz für diese konkrete Aufgabenstellung ist, bleibt dagegen offen und wird in Abschnitt~\ref{sec:fazit_gesamtbild} eingeordnet.
...
Der auffälligste Befund der Evaluation ist, dass eine ungelernte, primitive Heuristik das aufwendig trainierte RL-Modell in beiden Metriken deklassiert: Der $\varepsilon=0{,}8$-Kompass erreicht 88,8\,\% Erfolgsquote gegenüber 64,6\,\% und ein Wegverhältnis von 0,094 gegenüber 0,053 (siehe Abbildung~\ref{fig:eval_tradeoff}). Diese Heuristik wertet lediglich die Richtungskomponenten zum Ziel aus und geht einen Schritt auf der längeren Achse (siehe Abschnitt~\ref{sec:methodik_baselines}). Sie ist \glqq wandblind\grqq, nutzt keine Pfadfindung und kennt das Layout der Welt nicht.

Genau hier greifen die beiden Teile der Arbeit ineinander. Der Ausgang wird stets über eine Breitensuche vom Spawnpunkt aus platziert, wodurch die generierte Welt strukturell auf kurze, direkte Wegverbindungen hin ausgelegt ist (siehe Abschnitt~\ref{sec:umsetzung_pathing}). Die simple Kompass-Heuristik bewegt sich rein richtungsbasiert auf das Ziel zu. Naheliegend, aber noch nicht experimentell belegt, ist die Vermutung, dass die auf Erreichbarkeit optimierte Weltgenerierung genau diejenige einfache Wegstruktur begünstigt, die das ungelernte Verfahren instinktiv ausnutzt (siehe Abschnitt~\ref{sec:ausblick_rl}).

Daraus ergibt sich eine methodische Frage für weiterführende Arbeiten: Sollte sich diese Vermutung bestätigen, enthielte eine Trainingsumgebung, deren Lösbarkeit über geometrisch regelmäßige Konstruktionsprinzipien erzwungen wird, systembedingte Abkürzungen. Ein Vergleich zwischen gelernten und ungelernten Verfahren würde dann nicht mehr nur die \glqq Intelligenz\grqq\ des Reinforcement Learnings messen, sondern auch, wie gut sich solche strukturellen Eigenschaften der Welt ausnutzen lassen.

Für die anfängliche Fragestellung bedeutet das: LSTM-PPO bleibt der Gewinner unter den Deep-Learning-Verfahren, die Umgebung erfüllt ihre Performanzanforderungen. Bewiesen ist damit jedoch nicht, dass RL diese spezifische Navigationsaufgabe gut löst. Belegt ist stattdessen, dass RL unter diesen Rahmenbedingungen schlechter abschneidet als ein triviales Skript. Ob dahinter ein struktureller Vorteil der Kompass-Heuristik durch die Bauart der Weltgenerierung steht, bleibt eine offene, in Abschnitt~\ref{sec:ausblick_rl} benannte Frage.
...
\begin{enumerate}
    ...
    \item \textbf{Verbesserung der Pfad-Ästhetik (\textit{A*-Guided Carving}):} Der aktuelle Manhattan-Carver neigt stellenweise zu rechtwinkligen „L-Shapes“. Eine Integration eines A*-Algorithmus mit geglätteter Kostenfunktion (\textit{Bézier-Kurven} oder Diagonalschneiden) könnte die Wegführung noch organischer in das Noise-Terrain einbetten.
...
\textbf{1. Isolierung der Pfadgarantie:} Die Hypothese, dass die Kompass-Heuristik von der auf Erreichbarkeit optimierten, tendenziell direkten Ausgangsplatzierung profitiert, müsste experimentell bewiesen werden. Zukünftige Arbeiten sollten die Lösbarkeit über organische, irreguläre Pfadfindungsalgorithmen sicherstellen, da sich der wahre Leistungsunterschied zwischen Heuristik und RL-Agent erst isolieren lässt, wenn die Umgebung keine offensichtliche Geometrie mehr aufweist (vgl. Abschnitt~\ref{sec:umsetzung_pathing}).
...
\bigskip
Zum Schluss lässt sich das Ergebnis der Arbeit in drei Sätzen festhalten. Die entwickelte Weltgenerierung liefert eine performante, deterministische und garantiert lösbare Trainingsumgebung. Das Reinforcement Learning navigiert darin mit Gedächtnis zuverlässiger als ohne, bleibt aber hinter einer trivialen Kompass-Heuristik zurück. Ob dieser Rückstand an den Verfahren selbst oder an der Bauart der Welt liegt, könnte erst eine Umgebung zeigen, deren Lösbarkeit nicht über direkte Wege erzwungen wird; sie zu bauen wäre der naheliegende nächste Schritt.
...
% BEGIN: anhang.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Konfigurationsauszug}
\label{chap:anhang_config}

Listing~\ref{lst:anhang_config} zeigt den für die Weltgenerierung relevanten Ausschnitt der ausgelieferten Konfigurationsdatei, auf den Tabelle~\ref{tab:config_status} Bezug nimmt.

\begin{lstlisting}[language=json, caption={Weltgenerierungs-Konfiguration (Auszug)}, label={lst:anhang_config}]
"worldgen": {
    "exitMinDistance": 35,
    "exitMaxDistance": 45,
    ...
    "procedural": {
        ...
        "cellularBirthMinNeighbors": 5,
        "cellularSurvivalMinNeighbors": 4,
        "enableFloodFillValidation": false,
        "validationRadiusChunks": 8,
        "enableMacroGraphPrecheck": false,
        "macroGraphRadiusChunks": 16
    }
}
\end{lstlisting}

\chapter{Autorschaftsübersicht}
\label{chap:anhang_autorschaft}

Tabelle~\ref{tab:autorschaft} ordnet jedes Kapitel beziehungsweise jeden Abschnitt der Arbeit einer Person zu oder weist ihn als gemeinsam verfasst aus, ergänzend zur Erklärung im Vorspann.

\begin{table}[htbp]
    \centering
    \caption{Zuordnung der Kapitel und Abschnitte der Arbeit}
    \label{tab:autorschaft}
    \begin{tabular}{p{2.6cm}p{7.8cm}p{3.0cm}}
        \toprule
        \textbf{Kapitel} & \textbf{Titel} & \textbf{Person} \\
        \midrule
        1 & Einleitung & gemeinsam \\
        2 & Grundlagen & gemeinsam \\
        3.1 & Methodik der Weltgenerierung & Laurin \\
        3.2 & Methodik des RL-Trainings & Florian \\
        4.1 & Implementierung der Weltgenerierung & Laurin \\
        4.2 & Implementierung des RL-Trainings & Florian \\
        5.1 & Evaluation der Weltgenerierung & Laurin \\
        5.2 & Evaluation des RL-Trainings & Florian \\
        6.1 & Diskussion der Ergebnisse & gemeinsam \\
        6.2.1 & Beitrag der Weltgenerierung & Laurin \\
        6.2.2 & Beitrag des RL-Trainings & Florian \\
        6.2.3 & Gesamtbild & gemeinsam \\
        6.3.1 & Ausblick Weltgenerierung & Laurin \\
        6.3.2 & Ausblick Reinforcement Learning & Florian \\
        A & Konfigurationsauszug & Laurin \\
        B & Autorschaftsübersicht & gemeinsam \\
        C & Rohdaten der statistischen Auswertung & Florian \\
        \bottomrule
    \end{tabular}
\end{table}
...
% BEGIN: anhang_c.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Rohdaten der statistischen Auswertung}
\label{chap:anhang_rohdaten}

Dieser Anhang stellt die Einzelwerte bereit, auf denen die in Kapitel~5.2 berichteten Mittelwerte, Standardabweichungen und Signifikanztests beruhen, sowie die vollständige Ergebnistabelle der ungelernten Referenzverfahren aus Abschnitt~\ref{sec:eval_eff}.

\begin{table}[H]
    \centering
    \caption{LSTM-PPO: Erfolgsquote je Lauf, Testset A und Holdout B, stochastisch und deterministisch}
    \label{tab:rohdaten_lstm}
    \begin{tabular}{ccccc}
        \toprule
        \textbf{Seed} & \textbf{A stoch.\ (\%)} & \textbf{A det.\ (\%)} & \textbf{B stoch.\ (\%)} & \textbf{B det.\ (\%)} \\
        \midrule
        1 & 58,4 & 38,0 & 65,2 & 46,0 \\
        2 & 73,6 & 26,0 & 77,6 & 44,0 \\
        3 & 74,4 & 32,0 & 88,0 & 36,0 \\
        4 & 76,8 & 40,0 & 80,4 & 38,0 \\
        5 & 59,2 & 20,0 & 72,4 & 34,0 \\
        6 & 51,6 & 20,0 & 53,6 & 14,0 \\
        7 & 58,0 & 28,0 & 44,4 & 16,0 \\
        \midrule
        Mittel $\pm$ Std & 64,6 $\pm$ 10,1 & 29,1 $\pm$ 8,0 & 68,8 $\pm$ 15,5 & 32,6 $\pm$ 12,7 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{MLP-PPO: Erfolgsquote je Lauf, Testset A und Holdout B, stochastisch}
    \label{tab:rohdaten_mlp}
    \begin{tabular}{ccc}
        \toprule
        \textbf{Seed} & \textbf{A stoch.\ (\%)} & \textbf{B stoch.\ (\%)} \\
        \midrule
        1 & 18,8 & 17,6 \\
        2 & 53,6 & 62,8 \\
        3 & 22,0 & 20,8 \\
        ...
        7 & 65,2 & 69,2 \\
        \midrule
        Mittel $\pm$ Std & 33,5 $\pm$ 25,5 & 35,8 $\pm$ 30,5 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    \centering
    \small
    \caption{Ungelernte Referenzverfahren: Erfolgsquote, mittlere Schrittzahl und Wegverhältnis, Testset A und Holdout B}
    \label{tab:rohdaten_referenz}
    \begin{tabular}{lrrrrrr}
        \toprule
        & \multicolumn{3}{c}{\textbf{Testset A}} & \multicolumn{3}{c}{\textbf{Holdout B}} \\
        \textbf{Verfahren} & \textbf{SR (\%)} & \textbf{Schritte} & \textbf{Wegverh.} & \textbf{SR (\%)} & \textbf{Schritte} & \textbf{Wegverh.} \\
        \midrule
        Random & 5,2 $\pm$ 2,7 & 2251,9 & 0,021 & 10,4 $\pm$ 1,7 & 2031,2 & 0,025 \\
        Kompass $\varepsilon=0{,}3$ & 52,0 $\pm$ 4,2 & 481,0 & 0,177 & 50,4 $\pm$ 2,6 & 583,9 & 0,156 \\
        Kompass $\varepsilon=0{,}5$ & 71,6 $\pm$ 3,8 & 474,3 & 0,150 & 70,8 $\pm$ 4,8 & 520,5 & 0,148 \\
        Kompass $\varepsilon=0{,}6$ & 76,8 $\pm$ 7,0 & 482,8 & 0,138 & 80,4 $\pm$ 4,6 & 518,3 & 0,144 \\
        Kompass $\varepsilon=0{,}8$ & 88,8 $\pm$ 5,2 & 672,5 & 0,094 & 87,6 $\pm$ 1,7 & 606,3 & 0,092 \\
        Kompass $\varepsilon=0{,}9$ & 92,0 $\pm$ 5,1 & 1230,3 & 0,047 & 91,6 $\pm$ 4,3 & 1151,3 & 0,047 \\
        \bottomrule
    \end{tabular}
\end{table}

Alle stochastischen Werte sowie die Werte der Referenzverfahren sind Mittelwerte über fünf unabhängige Wiederholungen mit je eigenem Politik-RNG-Seed (siehe Abschnitt~\ref{sec:methodik_protokoll}); die deterministischen Spalten entstammen dagegen einer einmaligen Zusatzmessung je Lauf auf denselben Testset-Seeds, erkennbar am 2-Prozent-Raster der Einzelwerte. Die Standardabweichungen bei Random und den Kompass-Varianten beziehen sich auf die Streuung zwischen den fünf Wiederholungen, nicht auf die Streuung zwischen einzelnen Modell-Seeds wie bei LSTM-PPO und MLP-PPO.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% END: anhang_c.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
