...
erklärt, dass Kapitel~3.1, 4.1 sowie die Abschnitte~5.2 bis~5.5 von Laurin~\dots{}
und Kapitel~3.2, 4.2 sowie die Abschnitte~5.6 bis~5.10 von Florian~\dots{}
verfasst wurden; die gemeinsamen Kapitel~1, 2, 5.1, 6 und~7 entstanden in
...
\paragraph{Konkret unterteilen wir das in zwei Teilfragen:}
...
\begin{enumerate}
    \item \textbf{Recherche zu Weltgenerierung und RL-Algorithmen:} Zuerst recherchieren wir, wie eine prozedurale Generierung sinnvoll aufgebaut sein sollte und welche Algorithmen sich eignen, um zufällige, aber immer lösbare 2D-Welten zu erzeugen. Parallel dazu schauen wir uns an, welche RL-Algorithmen für Navigationsaufgaben mit eingeschränkter Sicht überhaupt infrage kommen.
    \item \textbf{Prototyping der Spielumgebung:} Auf dieser Basis bauen wir einen ersten Prototyp der Spielwelt. Diesen verbessern wir iterativ, bis Parameter wie Kartengröße, Wanddichte und die Sichtweite des Agenten gut zusammenpassen und die Level fair bleiben.
    \item \textbf{Training der RL-Agenten:} Sobald die Spielumgebung stabil steht, setzen wir die ausgewählten RL-Algorithmen darauf an, trainieren sie und passen die Hyperparameter Schritt für Schritt an.
    \item \textbf{Evaluation und Auswertung:} Zum Schluss testen wir die fertig trainierten Agenten auf vielen neu generierten Leveln. Wir erfassen die Ergebnisse – wie die Erfolgsquote und die Pfadeffizienz – systematisch und vergleichen die Verfahren miteinander.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Fokus auf die eigene Spielwelt:} Wir untersuchen ausschließlich unsere selbst entwickelte 2D-Gitterwelt. Eine Übertragbarkeit auf 3D-Umgebungen oder andere externe Benchmarks testen wir nicht.
    \item \textbf{Kein RL für die Weltgenerierung:} Reinforcement Learning wird bei uns nur für das Navigationsverhalten des Agenten eingesetzt. Die Spielwelt selbst wird über feste, klassische Algorithmen erzeugt (kein gelerntes Weltdesign).
    \item \textbf{Begrenzte Parameterstudie:} Wir führen keine unendliche Testreihe über alle denkbaren Kombinationen aus Wanddichten oder Kartengrößen durch, sondern konzentrieren uns auf die ausgewählten RL-Verfahren und praxisnahe Welteinstellungen.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Gitterbasierte Testwelten (MiniGrid \& Crafter):} Umgebungen wie \textit{MiniGrid} \cite{chevalier2023minigrid} oder \textit{Crafter} \cite{hafner2021crafter} verfolgen ein ganz ähnliches Prinzip wie unser Projekt. Sie bieten leichtgewichtige 2D-Gitterwelten, in denen der Agent oft nur einen begrenzten Ausschnitt der Karte sieht und einfache Navigations- oder Sammelaufgaben lösen muss.
    \item \textbf{Generalisierung durch prozedurale Level (Procgen):} Der \textit{Procgen Benchmark} \cite{cobbe2020procgen} untersucht gezielt, wie gut RL-Agenten auf immer wieder neu generierten Leveln verallgemeinern. Das bestätigt unsere Grundidee, dass prozedurale Welten verhindern, dass ein Agent feste Pfade einfach nur auswendig lernt.
    \item \textbf{Partielle Beobachtbarkeit und Gedächtnis (POPGym):} Besonders nah an unserem RL-Teil ist \textit{POPGym} \cite{morad2023popgym}. Die Autoren untersuchen dort ganz gezielt Umgebungen mit eingeschränkter Sicht (partielle Beobachtbarkeit) und vergleichen, wie sich Modelle mit Gedächtnis (rekurrente Policies) gegen Modelle ohne Gedächtnis schlagen. Das ist exakt der Ansatz, den wir auch für den Vergleich unserer Trainingsverfahren übernehmen.
...
\section{Grundlagen der prozeduralen Weltengenerierung}
\label{chap:grundlagen_procedural}

Dieses Kapitel legt die theoretischen und mathematischen Grundlagen für die prozedurale Generierung virtueller Spielwelten dar. Unter prozeduraler Generierung versteht man Verfahren, bei denen digitale Inhalte nicht manuell durch Designer erstellt, sondern algorithmisch zur Laufzeit berechnet werden. Dies ermöglicht die Erzeugung sehr großer oder theoretisch unendlicher Spielwelten bei geringem Speicherbedarf.

\section{Deterministischer Zufall und Pseudozufallszahlen}
\label{sec:grundlagen_zufall}

In der Informatik ist echter Zufall (\textit{True Randomness}) ohne spezielle Hardware-Schnittstellen schwer zu realisieren. Stattdessen kommen Pseudozufallszahlengeneratoren (\textit{Pseudo-Random Number Generators}, PRNG) zum Einsatz. Ein wesentliches Merkmal für die Weltengenerierung ist der Determininismus:
...
\begin{itemize}
    ...
    \item \textbf{Hash- und Mix-Funktionen:} Um räumliche Daten (wie zweidimensionale Koordinaten $(x, y)$) unabhängig von einem fortlaufenden Zustand abzufragen, werden häufig zustandslose Hash-Funktionen genutzt. Diese bilden Raumkoordinaten, den Seed und ein optionales \textit{Salt} (ein zusätzlicher Offset zur Unterscheidung verschiedener Abfragen) auf einen pseudozufälligen Wert im Intervall $[0, 1]$ ab.
\end{itemize}

\section{Gradienten- und Value-Noise-Verfahren}
...
\subsection{Value Noise und Interpolation}
Beim \textit{Value Noise} werden an den Knotenpunkten eines regulären Gitters pseudozufällige Skalarwerte bestimmt. Die Werte für Punkte innerhalb einer Gitterzelle berechnen sich anschließend durch Interpolation der umliegenden Knotenwerte.
...
\subsection{Domain Warping und fraktale Akkumulation}
...
\begin{itemize}
    ...
    \item \textbf{Domain Warping:} Hierbei werden die Eingangskoordinaten $(x, y)$ einer Rauschfunktion selbst durch den Ausgangswert einer anderen Rauschfunktion verzerrt:
    \begin{equation}
...
\section{Zelluläre Automaten zur Gefügeglättung}
...
\section{Graphentheoretische Erreichbarkeitsanalyse}
...
\subsection{Breitensuche (Breadth-First Search, BFS)}
Zur Analyse der Konnektivität wird das Zellgitter als ungerichteter Graph interpretiert, in dem begehbare Zellen als Knoten und Nachbarschaftsbeziehungen als Kanten dargestellt werden. Die Breitensuche (BFS) traversiert den Graphen ebenenweise vom Startknoten aus. Sie eignet sich zur Prüfung der Erreichbarkeit eines Zielknotens sowie zur Bestimmung des kürzesten Pfades auf ungewichten Gittern.

\subsection{Heuristische Validierung und Korrekturverfahren}
...
\paragraph{Partially Observable Markov Decision Processes (POMDP)}
...
\paragraph{Rekurrente Neuronale Netze (RNN)}
...
Für zusammenhängende Strukturen wird ein \textit{Value-Noise}-Verfahren verwendet (\texttt{world.cpp}, Z.~131). Die Werte der vier Eckpunkte einer Gitterzelle werden ermittelt und bilinear interpoliert (\texttt{world.cpp}, Z.~149). Zur Kantenenglättung wird der Interpolationsparameter $t \in [0, 1]$ zuvor über die Smoothstep-Funktion transformiert:
...
Zur topologischen Prüfung dient eine Breitensuche (Breadth-First Search, BFS). Sie verifiziert die Erreichbarkeit innerhalb des Chunks (\texttt{world.cpp}, Z.~555) sowie valide Ausgänge in einem definierten Distanzring (\texttt{world.cpp}, Z.~328).
...
Das Fundament der deterministischen Landschaftserzeugung bildet ein zweistufiger Prozess: Zunächst werden diskrete Raumkoordinaten deterministisch in Pseudozufallswerte transformiert, welche anschließend durch ein bilineares Value-Noise-Verfahren kontinuierlich interpoliert werden.
...
In Listing~\ref{lst:manhattan_carver} ist der relevante Ausschnitt aus der Methodenreihenfolge in \texttt{reset()} sowie die vollständige Implementierung der Methode \texttt{carveGuaranteedPath} dargestellt.
...
\label{sec:impl_rl}
Nachdem die technische Grundlage unserer Spielumgebung gelegt wurde, widmen wir uns nun der Umsetzung des eigentlichen RL-Trainings. In diesem Abschnitt geht es primär um das \glqq Wie\grqq: Wir schauen uns konkrete Code-Architekturen, Werte und Implementierungsdetails an. Die methodischen Gründe für diese Entscheidungen – das \glqq Warum\grqq\ – werden an dieser Stelle vorausgesetzt. Maßgeblich für alle hier genannten Werte und Logiken ist ausschließlich der implementierte Quellcode.

\subsection{Technologie-Stack}
\label{sec:impl_rl_stack}
Unsere gesamte Trainingspipeline ist in Python geschrieben. Um dennoch die Performance unseres C++-Simulationskerns zu nutzen, greifen wir über ein kompiliertes Modul darauf zu. Tabelle~\ref{tab:rl_stack} gibt einen kurzen Überblick über die verwendeten Werkzeuge.
...
Wir verwenden \texttt{sb3-contrib} als direkte Ergänzung zum Stable-Baselines3-Kernframework. Der Grund dafür ist pragmatisch: Das Standard-Framework bringt von Haus aus keine rekurrente Policy mit. Genau diese benötigen wir aber für unseren LSTM-Ansatz (RecurrentPPO). Obwohl das Framework auch Algorithmen wie DQN und A2C anbietet, konzentrieren wir uns beim Training ausschließlich auf den Vergleich von PPO und RecurrentPPO. Um den Durchsatz zu maximieren, simulieren wir konstant 16 Instanzen der Umgebung parallel.

\subsection{Anbindung der Spielumgebung und Aufbau der Beobachtung}
\label{sec:impl_rl_env}
Damit ein RL-Framework überhaupt mit unserer C++-Welt kommunizieren kann, benötigen wir einen standardisierten Übersetzer. Diese Aufgabe übernimmt die Python-Klasse \texttt{StoneforgeWorldEnv}. Sie implementiert die bekannte \texttt{gymnasium}-Schnittstelle mit \texttt{reset()} und \texttt{step()}, wandelt die Aktionen des Agenten in Simulationsschritte um und baut aus dem Spielzustand den Beobachtungsvektor zusammen. 

% TODO: Hier wörtlichen Quellcode aus Environment-Klasse (Zeilen 175-290) einfügen
\begin{lstlisting}[language=Python, caption={Aufbau des Beobachtungsvektors (Platzhalter für wörtlichen Code)}, label={lst:env_observation}]
# Der Vektor umfasst exakt 229 Dimensionen:
# [ Gitter (225) | HP | exitDx | exitDy | step_frac ]
class StoneforgeWorldEnv(gym.Env):
    # ...
    def _build_observation(self):
        window = self._sim.observe(self._radius)        
        tiles = window.astype(np.float32).ravel() / TILE_CODE_MAX
        
        extras = np.array([                             
            self._sim.hitpoints() / MAX_HP,
            np.clip(self._sim.exit_dx() / 64.0, -1.0, 1.0),
            np.clip(self._sim.exit_dy() / 64.0, -1.0, 1.0),
            self._sim.steps_taken() / self._step_limit,
        ], dtype=np.float32)
        
        return np.concatenate([tiles, extras])
\end{lstlisting}

Die in Listing~\ref{lst:env_observation} definierte Struktur gießt unsere Problemformulierung in Code:
...
\begin{itemize}
    ...
    \item \textbf{Das wandernede Sichtfenster:} Der Agent sieht immer nur den Ausschnitt direkt um sich herum. Dieser Kniff sorgt erst dafür, dass wir es hier mit einem partiell beobachtbaren Entscheidungsprozess (POMDP) zu tun haben.
\end{itemize}
...
% TODO: Hier wörtlichen Quellcode aus Trainingsskript (Zeilen 70-113) einfügen
\begin{lstlisting}[language=Python, caption={Instanziierung der Modelle mit asymmetrischen Parametern (Platzhalter für wörtlichen Code)}, label={lst:modelle}]
lstm_ppo = RecurrentPPO(
    "MlpLstmPolicy", env,
    n_steps=256, n_epochs=10, batch_size=8,
    policy_kwargs=dict(
        n_lstm_layers=1, lstm_hidden_size=256, 
...
mlp_ppo = PPO(
    "MlpPolicy", env,
    n_steps=256, n_epochs=10, batch_size=256,
...
% TODO: Hier wörtlichen Quellcode aus Trainingsskript (Zeilen 53-68 & 421 ff.) einfügen
\begin{lstlisting}[language=Python, caption={Curriculum-Phasen mit dualen Abbruchbedingungen (Platzhalter für wörtlichen Code)}, label={lst:curriculum_gating}]
PHASES = [
    Phase(train_dist=(5, 12), eval_dist=(5, 12), budget=500_000, target_sr=0.85),
    Phase(train_dist=(12, 25), eval_dist=(12, 25), budget=500_000, target_sr=0.70),
    Phase(train_dist=(25, 45), eval_dist=(35, 45), budget=1_000_000, target_sr=0.70),
    Phase(train_dist=(25, 45), eval_dist=(35, 45), budget=200_000, target_sr=0.60),
]
\end{lstlisting}
...
Um dem Agenten den Übergang in schwerere Phasen zu erleichtern, senken wir den Entropie-Koeffizienten (\texttt{ent\_coef}) in Phase 3 über $500\,000$ Schritte stetig von $0{,}05$ auf $0{,}001$ ab (Annealing). In Phase 4 bleibt er fix bei $0{,}0001$, wodurch der Agent von einem explorativen nahezu vollständig in ein deterministisches Verhalten übergeht.
...
% TODO: Hier wörtlichen Quellcode aus Simulationskern (Zeilen 1459-1525) einfügen!
\begin{lstlisting}[language=C++, caption={Berechnung des Rewards und Shaping-Terms im C++-Kern (Platzhalter für wörtlichen Code)}, label={lst:reward_shaping}]
// Ausschnitt aus der Reward-Berechnung in C++
float reward = 0.0f;
reward -= 0.01f; // Grundstrafe pro Schritt

if (is_new_tile) reward += 0.02f;
if (action == WAIT) reward -= 0.04f;
// ... (Schaden und Zwei-Schritt-Schleife)

if (reached_exit) {
    return 100.0f + shaping; // Terminal Reward
} else if (timeout) {
    return -10.0f;
}
\end{lstlisting}
...
Um unsere trainierten RL-Modelle bewerten zu können, brauchen wir ungelernte Referenzwerte (Baselines). Diese greifen direkt auf die Spiellogik zu und erfordern keine Trainingsphase. 

% TODO: Hier wörtlichen Quellcode aus Baseline-Skript (Zeilen 58-86) einfügen!
\begin{lstlisting}[language=Python, caption={Aktionswahl der Referenzverfahren (Platzhalter für wörtlichen Code)}, label={lst:baselines}]
# Auszug aus der Kompass-Heuristik
def compass_policy(obs, rng, epsilon):
...
Um die vollständige Reproduzierbarkeit zu gewährleisten, sind in Tabelle~\ref{tab:parameter_training} alle verbleibenden, fest im Code verankerten Hyperparameter für den Optimierer (PPO) sowie die generellen Testbedingungen aufgeführt. Wir stoppen Episoden regulär nach einem Limit von 4000 Schritten ab. Zusätzlich erzwingt das System einen Early-Stop, wenn der Agent 256 Schritte lang keine positive Belohnung sammelt. Um Varianzen durch die Generierung auszugleichen, mitteln wir die Ergebnisse über sieben unterschiedliche Zufallssaaten (Seeds).
...
\begin{enumerate}
    \item \textbf{Mindestausdehnung von Biomen (\textit{Clustering}):} Bei der Biom-Zuweisung auf Ebene des Chunk-Rasters muss durch eine zelluläre Automaten-Glättung sichergestellt werden, dass Biome stets in zusammenhängenden Clustern von mindestens $2 \times 2$ Chunks auftreten.
...
Das LSTM-PPO-Modell erreicht in der stochastischen Auswertung eine mittlere Erfolgsquote von 68,6\,\%. Über die sieben Durchläufe hinweg liegt die Standardabweichung bei 10,9 Prozentpunkten, die Einzelwerte schwanken zwischen 56,0\,\% und 88,0\,\%. Im deterministischen Modus bricht die Leistung massiv ein: Die Quote fällt auf magere 29,1\,\% $\pm$ 7,4.

Beim MLP-PPO-Verfahren fällt das Bild noch dramatischer aus, allerdings mit einer methodischen Einschränkung: Für das MLP-Modell liegt kein standardisierter Testset-A-Lauf vor (siehe Abschnitt~\ref{sec:methodik_protokoll}), die hier berichteten Zahlen stammen direkt aus den Zwischenevaluationen des Trainings. Stochastisch ausgewertet kommt das MLP-Modell auf 38,3\,\%, mit einer enormen Streuung von 28,6 Prozentpunkten. Deterministisch ausgewertet scheitert das Netz vollständig: Über alle sieben Läufe und alle 70 Messpunkte der späten Trainingsphasen hinweg liegt die Erfolgsquote bei exakt 0,0\,\%.

Der Systemvergleich fällt damit eindeutig aus: Die rekurrente LSTM-Architektur schlägt das gedächtnislose MLP-Netz deutlich. Im deterministischen Modus ist der Unterschied absolut, knapp jede dritte Episode gelöst gegenüber keiner einzigen. Im stochastischen Modus schrumpft der Vorsprung zwar auf rund 30 Prozentpunkte, und die starken Schwankungen des MLP-Modells sorgen für Überschneidungen. Das LSTM bleibt dennoch das klar überlegene Verfahren.
...
Das BFS-Optimum liegt auf dem verwendeten Testset im Schnitt bei 40,1 Schritten. Das trainierte LSTM-Modell benötigt für eine erfolgreiche Episode dagegen durchschnittlich 1429 Schritte, das entspricht einer Pfadeffizienz von lediglich 0,049. Der Agent läuft damit im Schnitt einen Weg, der mehr als 35-mal so lang ist wie nötig. Für das MLP-Modell liegt mangels Testset-A-Lauf kein vergleichbarer Wert vor.

Aufschlussreich wird der Blick auf die Referenzverfahren aus Abschnitt~\ref{sec:methodik_baselines}. Der ungelernte Kompass-Agent mit einem Zufallsanteil von $\varepsilon=0{,}3$ erreicht eine Effizienz von 0,158 und ist damit gut dreimal so wegeffizient wie das trainierte LSTM-Netz, auch wenn er dabei eine knapp 19 Prozentpunkte niedrigere Erfolgsquote in Kauf nimmt. Das überraschendere Ergebnis liefert der $\varepsilon=0{,}8$-Kompass: Er übertrifft das trainierte Modell nicht nur bei der Erfolgsquote, sondern mit einer Pfadeffizienz von 0,081 gleichzeitig auch bei der Weglänge.
...
Aus den Erkenntnissen und methodischen Einschränkungen der Arbeit ergeben sich drei Ansatzpunkte für weiterführende Projekte.
...
\textbf{2. Analyse der Determinismus-Lücke:} Offen bleibt, warum die Modelle stochastisch so viel besser abschneiden als deterministisch (siehe Abschnitt~\ref{sec:eval_determinismus}). Naheliegend ist die Vermutung, dass der Agent in lokale Optima gerät, etwa Zwei-Schritt-Schleifen in Sackgassen, aus denen ihn nur der Zufallsanteil beim Ziehen aus der Aktionsverteilung befreit. Eine visuelle Analyse der tatsächlich gelaufenen Trajektorien könnte zeigen, ob das Modell den Zufall nutzt, um Sackgassen zu entkommen, oder ob es das begrenzte Sichtfenster iterativ \glqq abtastet\grqq.
