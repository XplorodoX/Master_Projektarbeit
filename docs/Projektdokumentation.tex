...
deterministisch (Argmax). Details und Endergebnisse in
...
\subsection{Protokoll und Metriken}

Zentrale Metrik ist die \emph{Success Rate} (SR): der Anteil von 50 festen,
trainingsfremden Welten, in denen der Agent den Exit innerhalb von 4\,000
Schritten erreicht. Gemessen wird stets in beiden Betriebsarten (stochastisch
und deterministisch) und mit dem vollen Episodenlimit von 4\,000 Schritten ---
ein zu kurzes Evaluationslimit verfälscht die SR erheblich (dasselbe Modell:
48\,\% bei Limit 600 gegenüber 86\,\% bei 4\,000;
Abschnitt~\ref{sec:cap}). Als Diagnosemetrik des Trainings dient die
\texttt{explained\_variance} des Critics. Für das Endergebnis werden drei
unabhängige Läufe (Seeds 1--3) gemittelt und mit Standardabweichung angegeben.
...
Holdout B, je 50 Seeds, Limit 4\,000).}
\label{tab:v12}
\end{table}

Beide Zielkriterien sind damit im Mittel erfüllt (A: $73{,}3\,\% \geq 70\,\%$;
B: $80{,}0\,\% \geq 60\,\%$), erstmals in der geforderten Form mit drei Läufen
und Standardabweichung. Dass B leicht über A liegt, ist eine Eigenschaft der
...
\subsection{Analyse des Det/Stoch-Gaps nach Weglänge}
...
offen & $\geq$3 volle Curriculum-Läufe (batch=8, Seeds 1--3), Det-Gap-Maßnahmen (P3) & --- \\
\bottomrule
\end{tabular}
...
\subsection{Batch-Größe: vermeintliche Validierung (06.07.) --- am Folgetag widerlegt}
...
erfüllt, und die Umgebung ist nach der v11-Revision methodisch sauber (präzise
Schwierigkeitssteuerung, policy-invariantes Shaping, kein Straf-Stacking, keine
toten Features). Die Forensik der Etappe~6 hat die Trainingskonfiguration
konsolidiert (batch=8, Cap-4000-Evals, kurvenbasierte Bewertung) und liefert
mit 84--88\,\% stochastischer / 52\,\% deterministischer Phase-1-SR die
besten Zwischenwerte des Projekts. \textbf{Offene Schritte:}

\begin{enumerate}
  \item \textbf{$\geq$3 volle Curriculum-Läufe} mit der konsolidierten
        Konfiguration (batch=8, Seeds 1--3) für Mittelwert $\pm$
        Standardabweichung; anschließend finaler Eval auf Testset A
        (7000--7049) und Holdout B (8000--8049).
  \item \textbf{Det/Stoch-Gap schließen:} Die erstmals hohe deterministische
        Phase-1-SR ($>$50\,\% statt bisher $\leq$26\,\%) legt nahe, dass der
        gesunde Critic auch das Phase-3-Annealing stabilisiert --- zu prüfen
        im vollen Lauf. Ergänzend: (a) Temperatur-Sweep der Evaluation;
        (b) Self-Imitation \cite{oh2018sil}.
  \item \textbf{Seed-Replay:} Der Erfolgs-Swarm zeigte im A/B-Test
        (Abschnitt~\ref{sec:etappe6}) keinen Phase-1-Effekt; ob PLR-Semantik
        \cite{jiang2021plr} in den späteren Phasen hilft, bleibt offen.
